...
/*
  ...
  Typical use would be for example Fp<long, 2147389441> which should be pretty
  fast.
 */
...
public:
  ...
  Fp(Tint const &_x, Tint const &_y) {
    Fp<Tint, P> x = _x;
    x /= Fp<Tint, P>(_y);
    num = x.num;
  }
...
public:
  // Initially, this was a Rational<Tint> with a fraction defined as x.first /
  // x.second
  ...
  void operator/=(Fp<Tint, P> const &x) { *this = *this / x; }
  void operator+=(Fp<Tint, P> const &x) { *this = *this + x; }
  void operator-=(Fp<Tint, P> const &x) { *this = *this - x; }
