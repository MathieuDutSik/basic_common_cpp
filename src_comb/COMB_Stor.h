...
// a[i]=a
template <typename Tint> struct DoubleList {
public:
  ...
  Tint find_first() const { return ListNext[MaxElement]; }
...
public:
  ...
  EncapsDynamicBitset(std::size_t const &eMax) : eVect(eMax) { TotalCount = 0; }
  ~EncapsDynamicBitset() {}
  std::size_t count() const { return TotalCount; }
  std::size_t find_first() const { return eVect.find_first(); }
  ...
  bool get(std::size_t const &pos) const { return eVect[pos]; }
  bool empty() const {
    if (TotalCount == 0)
      return true;
    return false;
...
template <typename T> struct StackStorage {
public:
  StackStorage() { TheSize = 0; }
