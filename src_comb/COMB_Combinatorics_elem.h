...
bool BinomialStdvect_Increment(int const &n, int const &k,
...
private:
  ...
  public:
    IteratorContain(Tidx const &eDim, Tidx const &eSize,
                    std::vector<Tidx> const &eV)
        : dim_iter(eDim), size_iter(eSize), V(eV) {}
    std::vector<Tidx> const &operator*() { return V; }
