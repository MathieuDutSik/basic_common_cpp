...
public:
  ...
  BlockIteration(int const &eDim, int const &eSize)
      : dim(eDim), size(eSize), eVect(dim, 0) {}
  int IncrementShow() {
    for (int i = 0; i < dim; i++)
      if (eVect[i] < size - 1) {
        eVect[i]++;
  ...
  std::vector<int> GetVect() const { return eVect; }
  int GetNbPoss() const {
    int eRet = 1;
    for (int i = 0; i < dim; i++)
      eRet *= size;
...
private:
  ...
  public:
    IteratorContain(int const &eDim, int const &eSize,
                    std::vector<int> const &eU)
        : dim_iter(eDim), size_iter(eSize), U(eU) {}
    std::vector<int> const &operator*() { return U; }
    ...
    bool operator!=(IteratorContain const &iter) {
      if (iter.dim_iter != dim_iter || iter.size_iter != size_iter ||
          iter.U.size() != U.size())
        return true;
...
public:
  ...
  BlockCppIterator(int const &eDim, int const &eSize)
      : dim(eDim), size(eSize) {}
  ...
  const_iterator begin() const { return {dim, size, std::vector<int>(dim, 0)}; }
...
public:
  ...
  BlockIterationMultiple(std::vector<int> const &VectSiz)
      : dim(VectSiz.size()), VectSiz(VectSiz) {}
  //
  ...
  public:
    IteratorContain(std::vector<int> const &VectSiz, std::vector<int> const &U)
        : dim_iter(VectSiz.size()), VectSiz(VectSiz), U(U) {}
    std::vector<int> const &operator*() { return U; }
    IteratorContain &operator++() {
      single_increase();
      return *this;
...
public:
  ...
  const_iterator cend() const { return IteratorContain(VectSiz, {}); }
  const_iterator begin() const {
    return IteratorContain(VectSiz, std::vector<int>(dim, 0));
  }
  const_iterator end() const { return IteratorContain(VectSiz, {}); }
...
template <typename Tint>
std::vector<int> ConvertNumberToVector(Tint const &val, int const &N,
                                       int const &len) {
...
template <typename Tint>
std::vector<int> ConvertNumberToPrimitiveVector(Tint const &val, int const &N,
                                                int const &len) {
  Tint valWork = val;
...
public:
  ...
  Face GetFace() const { return eFace; }
  int GetNbPoss() const {
    int eRet = 1;
    for (int i = 0; i < dim; i++)
      eRet *= 2;
...
std::vector<Tint> ListRepresentativeConnectedComponent(
    Tint const &len,
    std::function<std::vector<Tint>(Tint const &)> const &GetAdjacent) {
  TstorAll Vlist(len);
...
template <typename T> struct IteratorBinomial {
public:
  ...
  T size() const { return ListBinom(n, k); }
