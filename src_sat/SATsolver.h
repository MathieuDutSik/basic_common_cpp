#ifndef INCLUDE_SAT_SOLVER
#define INCLUDE_SAT_SOLVER
...
void SATenumeration(SATformulation const& eExpr, int const& MAX_ITER, std::function<int(Face const&)> const&f)
{
  Minisat::Solver S;
  int verb=2;
  S.verbosity=verb;
  ...
  while (S.solve()) {
    ...
    for (int i = 0; i < nbVar; i++) {
      ...
      if (S.modelValue(i) == l_False) // Curiously, this is how we should do it, it seems.
	eFace[i]=1;
    }
