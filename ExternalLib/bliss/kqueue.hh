...
template <class Type> KQueue<Type>::KQueue() {
  entries = 0;
  end = 0;
  head = 0;
  tail = 0;
...
template <class Type> void KQueue<Type>::init(const unsigned int k) {
  assert(k > 0);
  if (entries)
    free(entries);
...
template <class Type> void KQueue<Type>::clear() {
  head = entries;
  tail = head;
}

template <class Type> bool KQueue<Type>::is_empty() const {
  return (head == tail);
}

template <class Type> unsigned int KQueue<Type>::size() const {
  if (tail >= head)
    return (tail - head);
  return ((end - head) + (tail - entries));
}

template <class Type> Type KQueue<Type>::front() const { return *head; }

template <class Type> Type KQueue<Type>::pop_front() {
  Type *old_head = head;
  head++;
  if (head == end)
    head = entries;
...
template <class Type> void KQueue<Type>::push_front(Type e) {
  if (head == entries)
    head = end - 1;
  else
    head--;
...
template <class Type> void KQueue<Type>::push_back(Type e) {
  *tail = e;
  tail++;
  if (tail == end)
    tail = entries;
