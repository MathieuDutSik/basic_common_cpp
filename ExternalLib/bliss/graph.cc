...
void AbstractGraph::update_labeling_and_its_inverse(
    unsigned int *const labeling, unsigned int *const labeling_inv) {
  const unsigned int N = get_nof_vertices();
  unsigned int *ep = p.elements;
...
void AbstractGraph::cert_add(const unsigned int v1, const unsigned int v2,
  ...
  if (refine_compare_certificate) {
    ...
    if ((refine_equal_to_first == false) && (refine_cmp_to_best < 0))
      return;
  }
...
void AbstractGraph::search(const bool canonical, Stats &stats) {
  ...
  cr_level = 0;
  if (opt_use_comprec && nucr_find_first_component(cr_level) == true &&
  ...
  while (!search_stack.empty()) {
    ...
    if (!first_path_info.empty()) {
      ...
      if (opt_use_failure_recording && was_fp_cert_equal &&
    ...
    if (!p.is_discrete()) {
      ...
      if (opt_use_comprec) {
        ...
        if (p.nof_discrete_cells() ==
          ...
          if (canonical && !first_path_info.empty() &&
...
Digraph *Digraph::permute(const unsigned int *const perm) const {
  Digraph *const g = new Digraph(get_nof_vertices());
  for (unsigned int i = 0; i < get_nof_vertices(); i++) {
    const Vertex &v = vertices[i];
    g->change_color(perm[i], v.color);
...
void Digraph::write_dot(const char *const filename) {
...
unsigned int Digraph::vertex_color_invariant(const Digraph *const g,
                                             const unsigned int vnum) {
  return g->vertices[vnum].color;
}

unsigned int Digraph::indegree_invariant(const Digraph *const g,
                                         const unsigned int vnum) {
  return g->vertices[vnum].nof_edges_in();
}

unsigned int Digraph::outdegree_invariant(const Digraph *const g,
                                          const unsigned int vnum) {
  return g->vertices[vnum].nof_edges_out();
}

unsigned int Digraph::selfloop_invariant(const Digraph *const g,
                                         const unsigned int vnum) {
...
bool Digraph::refine_according_to_invariant(
    unsigned int (*inv)(const Digraph *const g, const unsigned int v)) {
  bool refined = false;
...
bool Digraph::split_neighbourhood_of_cell(Partition::Cell *const cell) {
  ...
  while (!neighbour_heap.is_empty()) {
    ...
    while (1) {
      if (in_search) {
        ...
        /* No need to continue? */
        if (refine_compare_certificate && (refine_equal_to_first == false) &&
...
bool Digraph::split_neighbourhood_of_unit_cell(
    Partition::Cell *const unit_cell) {
  ...
  for (unsigned int j = v.nof_edges_out(); j > 0; j--) {
    ...
    unsigned int *const swap_position = p.elements + neighbour_cell->first +
  ...
  while (!neighbour_heap.is_empty()) {
    ...
    if (neighbour_cell->length > 1 &&
      ...
      Partition::Cell *const new_cell = p.aux_split_in_two(
          neighbour_cell,
     ...
     */
    if (in_search) {
      for (unsigned int i = neighbour_cell->first, j = neighbour_cell->length;
           j > 0; j--, i++) {
        /* Build certificate */
...
bool Digraph::is_equitable() const {
  ...
  for (Partition::Cell *cell = p.first_cell; cell; cell = cell->next) {
    ...
    for (std::vector<unsigned int>::const_iterator ei =
         ...
         ei != first_vertex.edges_out.end(); ei++) {
  ...
  for (Partition::Cell *cell = p.first_cell; cell; cell = cell->next) {
    ...
    for (std::vector<unsigned int>::const_iterator ei =
         ...
         ei != first_vertex.edges_in.end(); ei++) {
      first_count[p.get_cell(*ei)->first]++;
    }
...
bool Digraph::is_automorphism(const std::vector<unsigned int> &perm) const {
...
bool Digraph::nucr_find_first_component(const unsigned int level) {
 ...
 * leaving the vertex.
 */
void Graph::Vertex::sort_edges() { std::sort(edges.begin(), edges.end()); }
...
void Graph::change_color(const unsigned int vertex, const unsigned int color) {
...
Graph *Graph::permute(const std::vector<unsigned int> &perm) const {
#if defined(BLISS_CONSISTENCY_CHECKS)
#endif
 ...
 * Time complexity: O(1)
 */
unsigned int Graph::vertex_color_invariant(const Graph *const g,
 ...
 * Time complexity: O(1)
 */
unsigned int Graph::degree_invariant(const Graph *const g,
                                     const unsigned int v) {
 ...
 */
unsigned int Graph::selfloop_invariant(const Graph *const g,
                                       const unsigned int v) {
...
bool Graph::refine_according_to_invariant(
    unsigned int (*inv)(const Graph *const g, const unsigned int v)) {
  bool refined = false;
...
bool Graph::nucr_find_first_component(const unsigned int level) {
