 ...
 */
template <class Type> class KStack {
public:
  /**
   ...
   */
  Type pop() { return *cursor--; }

  /**
   * Push the element \a e in the stack.
   */
  void push(Type e) { *(++cursor) = e; }
...
template <class Type> KStack<Type>::KStack() {
  kapacity = 0;
  entries = 0;
  cursor = 0;
}

template <class Type> KStack<Type>::KStack(int k) {
  assert(k > 0);
  kapacity = k;
...
template <class Type> void KStack<Type>::init(int k) {
  assert(k > 0);
  if (entries)
    free(entries);
  kapacity = k;
...
template <class Type> KStack<Type>::~KStack() { free(entries); }
