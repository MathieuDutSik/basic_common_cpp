...
Partition::BacktrackPoint Partition::set_backtrack_point() {
  BacktrackInfo info;
...
void Partition::goto_backtrack_point(BacktrackPoint p) {
...
Partition::Cell *Partition::individualize(Partition::Cell *const cell,
...
size_t Partition::print(FILE *const fp, const bool add_newline) const {
  size_t r = 0;
  const char *cell_sep = "";
  r += fprintf(fp, "[");
...
size_t Partition::print_signature(FILE *const fp,
...
void Partition::splitting_queue_add(Cell *const cell) {
...
void Partition::splitting_queue_clear() {
 ...
 */
Partition::Cell *Partition::sort_and_split_cell1(Partition::Cell *const cell) {
 ...
 */
bool Partition::shellsort_cell(Partition::Cell *const cell) {
  unsigned int h;
  unsigned int *ep;
...
Partition::Cell *Partition::zplit_cell(Partition::Cell *const cell,
...
unsigned int Partition::cr_get_backtrack_point() {
  assert(cr_enabled);
  CR_BTInfo info;
...
void Partition::cr_goto_backtrack_point(const unsigned int btpoint) {
  assert(cr_enabled);
...
void Partition::cr_create_at_level(const unsigned int cell_index,
...
void Partition::cr_create_at_level_trailed(const unsigned int cell_index,
                                           const unsigned int level) {
  assert(cr_enabled);
