...
static TLS_ATTR set w01[MAXM], w02[MAXM], w03[MAXM], w12[MAXM], w13[MAXM],
    w23[MAXM];
...
void twopaths(graph *g, int *lab, int *ptn, int level, int numcells, int tvpos,
...
void quadruples(graph *g, int *lab, int *ptn, int level, int numcells,
                int tvpos, int *invar, int invararg, boolean digraph, int m,
                int n) {
  int i, pc;
  setword sw;
  set *gw;
...
void triples(graph *g, int *lab, int *ptn, int level, int numcells, int tvpos,
             int *invar, int invararg, boolean digraph, int m, int n) {
  int i, pc;
  setword sw;
  set *gw;
...
void adjtriang(graph *g, int *lab, int *ptn, int level, int numcells, int tvpos,
               int *invar, int invararg, boolean digraph, int m, int n) {
  int j, pc;
  setword sw;
  set *gi;
...
void getbigcells(int *ptn, int level, int minsize, int *bigcells,
...
void celltrips(graph *g, int *lab, int *ptn, int level, int numcells, int tvpos,
               int *invar, int invararg, boolean digraph, int m, int n) {
  int i, pc;
  setword sw;
  set *gw;
...
void cellquads(graph *g, int *lab, int *ptn, int level, int numcells, int tvpos,
               int *invar, int invararg, boolean digraph, int m, int n) {
  int i, pc;
  setword sw;
  set *gw;
...
void cellquins(graph *g, int *lab, int *ptn, int level, int numcells, int tvpos,
               int *invar, int invararg, boolean digraph, int m, int n) {
  int i, pc;
  setword sw;
  set *gw;
...
static int uniqinter(set *s1, set *s2, int m) {
  int i, j;
  setword w;
...
void cellfano2(graph *g, int *lab, int *ptn, int level, int numcells, int tvpos,
               int *invar, int invararg, boolean digraph, int m, int n) {
  int i, pc;
  setword sw;
  int wt;
  ...
  for (icell = 0; icell < bigcells; ++icell) {
    ...
    for (iv0 = cell1; iv0 <= cell2 - 3; ++iv0) {
      ...
      for (iv1 = iv0 + 1; iv1 <= cell2; ++iv1) {
        ...
        if (ISELEMENT(gv0, v1))
          continue;
        if ((x01 = uniqinter(gv0, GRAPHROW(g, v1, m), m)) < 0)
          continue;
        vv[nw] = v1;
        ww[nw] = x01;
        ++nw;
        ...
        for (iv2 = iv1 + 1; iv2 < nw - 1; ++iv2) {
          ...
          for (iv3 = iv2 + 1; iv3 < nw; ++iv3) {
            ...
            if (ISELEMENT(gv1, v3) || ISELEMENT(gv2, v3))
              continue;
            gv3 = GRAPHROW(g, v3, m);
            if ((x13 = uniqinter(gv1, gv3, m)) < 0)
              continue;
            if ((x23 = uniqinter(gv2, gv3, m)) < 0 || x23 == x13)
              continue;

            if ((pnt0 = uniqinter(GRAPHROW(g, x01, m), GRAPHROW(g, x23, m),
                                  m)) < 0)
              continue;
            if ((pnt1 = uniqinter(GRAPHROW(g, x02, m), GRAPHROW(g, x13, m),
                                  m)) < 0)
              continue;
            if ((pnt2 = uniqinter(GRAPHROW(g, x03, m), GRAPHROW(g, x12, m),
                                  m)) < 0)
              continue;
...
void setnbhd(graph *g, int m, int n, set *w, set *wn) {
  int i, j;
  set *gi;
...
void cellfano(graph *g, int *lab, int *ptn, int level, int numcells, int tvpos,
              int *invar, int invararg, boolean digraph, int m, int n) {
  int i, pc;
  setword sw;
  int wt;
    ...
    for (iv0 = cell1; iv0 <= cell2 - 3; ++iv0) {
      ...
      for (iv1 = iv0 + 1; iv1 <= cell2 - 2; ++iv1) {
        ...
        for (iv2 = iv1 + 1; iv2 <= cell2 - 1; ++iv2) {
          ...
          for (iv3 = iv2 + 1; iv3 <= cell2; ++iv3) {
            v3 = lab[iv3];
            if (ISELEMENT(gv0, v3) || ISELEMENT(gv1, v3) || ISELEMENT(gv2, v3))
              continue;
            gv3 = GRAPHROW(g, v3, m);
            for (i = M; --i >= 0;)
              workset[i] = gv0[i] & gv3[i];
            setnbhd(g, m, n, workset, w03);
            for (i = M; --i >= 0;)
              workset[i] = gv1[i] & gv3[i];
            setnbhd(g, m, n, workset, w13);
            for (i = M; --i >= 0;)
              workset[i] = gv2[i] & gv3[i];
            setnbhd(g, m, n, workset, w23);

            for (i = M; --i >= 0;)
              workset[i] = w01[i] & w23[i];
            setnbhd(g, m, n, workset, pt0);
            for (i = M; --i >= 0;)
              workset[i] = w03[i] & w12[i];
            setnbhd(g, m, n, workset, pt1);
            for (i = M; --i >= 0;)
              workset[i] = w02[i] & w13[i];
...
void distances(graph *g, int *lab, int *ptn, int level, int numcells, int tvpos,
               int *invar, int invararg, boolean digraph, int m, int n) {
  int i;
  set *gw;
  int wt;
...
void indsets(graph *g, int *lab, int *ptn, int level, int numcells, int tvpos,
             int *invar, int invararg, boolean digraph, int m, int n) {
  int i;
  int wt;
  set *gv;
...
void cliques(graph *g, int *lab, int *ptn, int level, int numcells, int tvpos,
             int *invar, int invararg, boolean digraph, int m, int n) {
  int i;
  int wt;
  set *gv;
...
void cellcliq(graph *g, int *lab, int *ptn, int level, int numcells, int tvpos,
              int *invar, int invararg, boolean digraph, int m, int n) {
  int i;
  int wt;
  set *gv;
  ...
  cellstart = workshort;
  cellsize = workshort + (n / 2);
  getbigcells(ptn, level, setsize > 6 ? setsize : 6, &bigcells, cellstart,
              cellsize, n);
      ...
      while (ss > 0) {
        if (ss == setsize) {
          for (i = ss; --i >= 0;)
            ++invar[v[i]];
          --ss;
        } else if ((v[ss] = nextelement((set *)wss + M * (ss - 1), M, v[ss])) <
                   0)
          --ss;
        else {
          ++ss;
...
void cellind(graph *g, int *lab, int *ptn, int level, int numcells, int tvpos,
             int *invar, int invararg, boolean digraph, int m, int n) {
  int i;
  int wt;
  set *gv;
...
void adjacencies(graph *g, int *lab, int *ptn, int level, int numcells,
                 int tvpos, int *invar, int invararg, boolean digraph, int m,
                 int n) {
  int i, v, w;
  int vwt, wwt;
...
void nautinv_check(int wordsize, int m, int n, int version) {
...
static int semirefine(graph *g, int *lab, int *ptn, int level, int *numcells,
...
void refinvar(graph *g, int *lab, int *ptn, int level, int numcells, int tvpos,
              int *invar, int invararg, boolean digraph, int m, int n) {
  int i, j;
  int wt;
  int icell, bigcells, cell1, cell2;
  ...
  for (icell = 0; icell < bigcells; ++icell) {
    ...
    for (i = cell1; i <= cell2; ++i) {
      ...
      vv[cell1] = lab[i];
      invar[lab[i]] =
          semirefine(g, vv, ww, level, &newnumcells, invararg, ws1, m, n);
    }
    wt = invar[lab[cell1]];
    for (i = cell1 + 1; i <= cell2; ++i)
      if (invar[lab[i]] != wt)
