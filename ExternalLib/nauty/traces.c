...
static boolean traces_degree_refine(sparsegraph *, Candidate *, int,
                                    Partition *, struct TracesVars *,
                                    struct TracesInfo *, int, int *);
static int traces_vertexclass_refine(int, int *, int *, Candidate *,
                                     Partition *, int *);
static int traces_refine(Candidate *, int, Partition *, struct TracesVars *,
                         struct TracesInfo *, int, boolean);
...
static int traces_refine_refine(sparsegraph *, Candidate *, int, Partition *,
                                struct TracesVars *, struct TracesInfo *);
static void refine_tr_refine(Candidate *, int, Partition *, struct TracesVars *,
                             struct TracesInfo *);
...
static int CheckForAutomorphisms(Candidate *, Candidate *, struct TracesVars *,
                                 struct TracesInfo *, int, int, Partition *);
static int CheckForSingAutomorphisms(Candidate *, Partition *, Candidate *,
                                     struct TracesVars *, struct TracesInfo *,
                                     int, int);
static int CheckForMatching(Candidate *, Candidate *, Partition *,
                            struct TracesVars *, struct TracesInfo *, int, int);
static void Individualize(Partition *, Candidate *, int, int, int, int);
static boolean TreeFyTwo(int, Candidate *, Candidate *, Partition *, int,
                         struct TracesVars *, struct TracesInfo *);
static void ExperimentalStep(Partition *, Candidate *, struct TracesVars *,
                             struct TracesInfo *, int, int);
static boolean TargetCell(Candidate *, Partition *, int, struct TracesVars *,
                          int);
static boolean TargetCellFirstPath(Candidate *, Partition *,
                                   struct TracesVars *);
static int TargetCellExpPath(Candidate *, Partition *, struct TracesVars *);
static boolean TargetCellSmall(Candidate *, Partition *, int,
                               struct TracesVars *, int);
static boolean TargetCellFirstPathSmall(Candidate *, Partition *,
                                        struct TracesVars *);
static int TargetCellExpPathSmall(Candidate *, Partition *,
                                  struct TracesVars *);
...
static int CompStage0(Partition *, Partition *, Candidate *, Candidate *, int,
                      int, struct TracesVars *, struct TracesInfo *);
static int CompStage1(Partition *, Partition *, Candidate *, Candidate *, int,
                      int, struct TracesVars *, struct TracesInfo *);
static int CompStage2(Partition *, Partition *, Candidate *, Candidate *, int,
                      int, struct TracesVars *, struct TracesInfo *);
static void grouporderplus(sparsegraph *, Candidate *, Partition *, permnode **,
                           double *, int *, int, struct TracesVars *,
                           struct TracesInfo *);
...
static searchtrie *searchtrie_make(Candidate *, Candidate *, int,
                                   struct TracesVars *);
static boolean lookup(searchtrie *);
static int *findcurrorbits(schreier *, int);
static int Preprocess(sparsegraph *, permnode **, Candidate *, int, Partition *,
                      struct TracesVars *);
static void MakeTree(int, int, sparsegraph *, int, struct TracesVars *,
                     boolean);
static void MakeCanTree(int, sparsegraph *, int, Candidate *, Partition *,
                        struct TracesVars *);
...
static boolean isautom_sg_pair(graph *, int *, boolean, int, int,
                               struct TracesVars *);
...
static int FirstNeighbour(int, Candidate *, Partition *, int *, int, int *,
                          int);
...
static void Complete(sparsegraph *, Candidate *, Partition *, int, TracesVars *,
                     double *, int *, permnode **, int);
static void Allocate_Traces_Structures(int);
static void Allocate_refine_Structures(int);
static void Initialize_Traces_Variables(TracesVars *, TracesOptions *,
                                        TracesStats *, int *, sparsegraph *,
                                        sparsegraph *, int);
...
#define SETMARK(Arr, Mrk)                                                      \
  if (Mrk > (NAUTY_INFINITY - 2)) {                                            \
...
#define PRINTCAND(V, Lev)                                                      \
  PRINTCHAR(" ") for (tmp = 1; tmp <= Lev; tmp++) {                            \
    fprintf(outfile, tv->digstring, V->lab[Spine[tmp].tgtpos] + labelorg);     \
  }

#define PRINTCANDF(V, Lev)                                                     \
  {                                                                            \
    NEXTLINE for (tmp = 1; tmp <= Lev; tmp++) {                                \
      fprintf(outfile, "F%di", V->lab[Spine[tmp].tgtpos] + labelorg);          \
    }                                                                          \
    NEXTLINE                                                                   \
  }

#define PRINTCANDBIG(V, Lev)                                                   \
  {                                                                            \
    PRINTCHAR(" ")                                                             \
    for (tmp = 1; tmp <= 5; tmp++) {                                           \
      fprintf(outfile, tv->digstring, V->lab[Spine[tmp].tgtpos] + labelorg);   \
...
#define LINE(K, c)                                                             \
  {                                                                            \
    PRINTCHAR(c) for (tmp = 1; tmp <= K; tmp++) { fprintf(outfile, c); }       \
  }

#define TRACE_CHECK(Tr, Ind, Arg, End)                                         \
  {                                                                            \
    TracePos = Tr + Ind;                                                       \
    if (newtrace) {                                                            \
      *TracePos = Arg;                                                         \
...
#define SAMETRACE_CHECK(Tr, Ind, Arg, End)                                     \
  {                                                                            \
    TracePos = Tr + Ind;                                                       \
    if (Ind < *End) {                                                          \
      if (*TracePos != Arg) {                                                  \
...
#define COPYPART(P, Q)                                                         \
  {                                                                            \
    memcpy(P->cls, Q->cls, n * sizeof(int));                                   \
    memcpy(P->inv, Q->inv, n * sizeof(int));                                   \
    P->cells = Q->cells;                                                       \
    P->code = Q->code;                                                         \
  }

#define ADDTONEXTLEVEL                                                         \
  {                                                                            \
    if (SpineTL->listend) {                                                    \
      (SpineTL->listend)->next = NewCandidate(n, &GarbList, TRUE);             \
      if ((tv->compstage < 2) &&                                               \
          (SpineTL->listcounter <= (NAUTY_INFINITY - 2)))                      \
        SpineTL->listcounter++;                                                \
...
#define ORBITSIZES                                                             \
  {                                                                            \
    memset(OrbSize, 0, n * sizeof(int));                                       \
    for (i = 0; i < n; i++) {                                                  \
      OrbSize[tv->orbits[i]]++;                                                \
    }                                                                          \
  }

#define CURRORBITSIZES                                                         \
  {                                                                            \
...
#define EXITFROMSTAGE0REFINE                                                   \
  {                                                                            \
    PRINT_LINE_PLUS(tv->tolevel)                                               \
...
#define EXITFROMSTAGE0EXPATH1                                                  \
  {                                                                            \
    PRINT_RETURN PRINT_LINE_PLUS(tv->tolevel) if (tv->options->verbosity >= 2) \
        fprintf(outfile, "==-");                                               \
...
#define PRINT_LINE                                                             \
  {                                                                            \
    if ((tv->options->verbosity >= 1) && (tv->strategy == 0)) {                \
      if (tv->options->verbosity >= 2) {                                       \
        LINE(tv->linelgth, "-");                                               \
        NEXTLINE                                                               \
      }                                                                        \
...
#define PRINT_LINE_PLUS(Lev)                                                   \
  {                                                                            \
    if ((tv->options->verbosity >= 1) && (tv->strategy == 0)) {                \
      ...
      fprintf(                                                                 \
          outfile,                                                             \
          " Level %d:  %d cell%s; %d singleton%s; target cell: %d; %d "        \
          "orbit%s; %lu node%s (%lu kept); %d update%s;",                      \
          Lev, SS(Spine[Lev].part->cells, "", "s"),                            \
          SS((Spine[Lev].part->cells == n) ? n : Spine[Lev].singend, "", "s"), \
          Spine[Lev].tgtsize, SS(tv->stats->numorbits, "", "s"),               \
          SS(Spine[Lev].levelcounter, "", "s"), Spine[Lev].keptcounter,        \
          SS(Spine[Lev].updates, "", "s"));                                    \
      if (Lev <= tv->group_level)                                              \
        fprintf(outfile, " group_level: %d", tv->group_level);                 \
      NEXTLINE                                                                 \
    }                                                                          \
  }
...
#define PRINT_CANDIDATEPLUS(PrevCand, Cand, Lev)                               \
  {                                                                            \
    ...
    cu = 14 - cu - cu1;                                                        \
    LINE(cu, "-")                                                              \
    fprintf(outfile, " %d, %d, %d) ", Lev % 10000, Cand->name % 10000000,      \
            PrevCand->name);                                                   \
    if (Lev < 12) {                                                            \
      PRINTCAND(Cand, Lev)                                                     \
    } else {                                                                   \
  ...
  {                                                                            \
    if (tv->options->verbosity >= 2) {                                         \
      if ((tv->tolevel_tl - tv->tolevel < 6) || !has_nexttcell) {              \
        ...
        if (Boo) {                                                             \
          if (tv->options->verbosity >= 2)                                     \
            fprintf(outfile, "{%d:%x} ", tv->tcellexpath, Cand->code);         \
        } else                                                                 \
          fprintf(outfile, "{interr.(%d)} ", NextPart->cells);                 \
...
#define PRINT_FROM_VERB(Verb, Lev)                                             \
  {                                                                            \
    if (tv->options->verbosity >= Verb) {                                      \
      ...
      fprintf(outfile,                                                         \
              " do_it: %d, indnum: %d, stnode->index: %d tcell @ %d: %d",      \
              CurrCand->do_it, CurrCand->indnum, CurrCand->stnode->index,      \
              tv->tcellevel, Spine[tv->tcellevel].tgtcell);                    \
      PRINT_RETURN                                                             \
    }                                                                          \
  }

#define PRINT_NOTMIN_VERB(Verb)                                                \
  {                                                                            \
    if (tv->options->verbosity >= Verb) {                                      \
      fprintf(outfile, " is NOT minimal in orbits (1, %d) [%d]; ", gom_level,  \
              CurrCand->lab[Spine[gom_level + 1].tgtpos] + labelorg);          \
      fprintf(outfile, "at lev %d, orb[%d] = %d.\n", gom_level + 1,            \
              CurrCand->lab[Spine[gom_level + 1].tgtpos] + labelorg,           \
              tv->currorbit[CurrCand->lab[Spine[gom_level + 1].tgtpos]] +      \
                  labelorg);                                                   \
    }                                                                          \
  }

#define PRINT_SKIPPED_VERB(Verb)                                               \
  {                                                                            \
    if (tv->options->verbosity >= Verb)                                        \
      fprintf(outfile, " skipped (0) (orbit[%d] = %d)\n",                      \
              NextCand->vertex + labelorg,                                     \
              tv->currorbit[NextCand->vertex] + labelorg);                     \
  }

#define PRINT_REFINE_VERB(Verb, where)                                         \
  {                                                                            \
    if (tv->options->verbosity >= Verb)                                        \
...
#define PRINT_INDIV_VERB(Verb, Lev)                                            \
  {                                                                            \
    if (tv->options->verbosity >= Verb) {                                      \
      if (Lev < 12) {                                                          \
        PRINTCAND(CurrCand, tv->fromlevel)                                     \
...
#define UPDATEMIN(A, B)                                                        \
  if (B < A)                                                                   \
    A = B;

#define PAIRORBJOIN(A, V)                                                      \
  {                                                                            \
    if (A != V) {                                                              \
...
#define SETPAIRSAUTANDTREE(arg, val)                                           \
  {                                                                            \
    if (tv->build_autom) {                                                     \
      SETPAIRSAUT(arg, val)                                                    \
    }                                                                          \
    if (arg != val)                                                            \
      orbjoin_sp_pair(tv->orbits, OrbList, n, arg, val,                        \
                      &tv->stats->numorbits);                                  \
    MakeTree(arg, val, sg_orig, n, tv, FALSE);                                 \
  }

#define PRINTF2(A, B)                                                          \
  if (tv->options->verbosity > 3)                                              \
  printf(A, B)
#define PRINTF2_2(A, B, C)                                                     \
  if (tv->options->verbosity > 3)                                              \
  printf(A, B, C)
#define PRINTF2_3(A, B, C, D)                                                  \
  if (tv->options->verbosity > 3)                                              \
  printf(A, B, C, D)
#define PRINTF2_4(A, B, C, D, E)                                               \
  if (tv->options->verbosity > 3)                                              \
  printf(A, B, C, D, E)

#define VERB_PRINT(V, Verb, R)                                                 \
  if (tv->options->verbosity >= Verb) {                                        \
...
void Traces(sparsegraph *g_arg, int *lab, int *ptn, int *orbits_arg,
            TracesOptions *options_arg, TracesStats *stats_arg,
  ...
  DYNFREE(g_arg->w,
          g_arg->wlen); /* to be removed in presence of weightd edges */

  Initialize_Traces_Variables(tv, options_arg, stats_arg, orbits_arg, g_arg,
                              canong_arg, n);
  ...
  tv->graph = &redgraph;
  if (g_arg->w)
    memcpy(tv->graph->w, g_arg->w, tv->graph->wlen * sizeof(int));
  ...
  if (ti->regular) {
    ...
    if (tv->options->weighted) {
      CurrPart->cells = traces_vertexclass_refine(n, lab, ptn, CurrCand,
                                                  CurrPart, WeightsSeq);
    } else {
      CurrPart->cells =
          traces_vertexclass_refine(n, lab, ptn, CurrCand, CurrPart, g_arg->d);
    }
  }

  memset(NghCounts, 0, n * sizeof(int));
  if (tv->options->verbosity == 7)
    PrintPartition(CurrCand->lab, CurrPart->cls, n, labelorg, 1323);
  ...
  if (CurrPart->cells == n) {
    ...
    if (tv->options->getcanon) {
      ...
      if (canong_arg)
        memcpy(CurrPart->inv, CurrCand->invlab, n * sizeof(int));
      if (tv->options->usercanonproc != NULL) {
        (*tv->options->usercanonproc)((graph *)g_arg, lab, (graph *)canong_arg,
                                      tv->stats->canupdates, CurrCand->code, m,
                                      n);
      }
    }
  } else {
    ...
    do {
      ...
      if (CurrCand) {
        ...
        case 0:
          retval =
              CompStage0(CurrPart, NextPart, CurrCand, NextCand, m, n, tv, ti);
          break;
        case 1:
          if (!TempOrbits) {
          ...
          retval =
              CompStage1(CurrPart, NextPart, CurrCand, NextCand, m, n, tv, ti);
          break;
        case 2:
          retval =
              CompStage2(CurrPart, NextPart, CurrCand, NextCand, m, n, tv, ti);
          break;
        default:
          break;
        }
    ...
    if (!retval) {
      ...
      if (tv->maxtreelevel)
        PRINT_LINE_PLUS(tv->maxtreelevel);
      ...
      if (tv->options->getcanon) {
        ...
        while (AuxCand) {
          if (AuxCand->do_it) {
            if (comparelab_tr(g_arg, BestCand->lab, BestCand->invlab,
                              AuxCand->lab, AuxCand->invlab,
                              Spine[tv->maxtreelevel].part->cls,
                              Spine[tv->maxtreelevel].part->inv) == 1) {
              BestCand = AuxCand;
              tv->stats->canupdates++;
              if (tv->options->usercanonproc != NULL) {
                (*tv->options->usercanonproc)(
                    (graph *)g_arg, lab, (graph *)canong_arg,
                    tv->stats->canupdates, CurrCand->code, m, n);
              }
            }
          }
          AuxCand = AuxCand->next;
        ...
        grouporderplus(g_arg, BestCand, Spine[tv->maxtreelevel].part, &gensB,
                       &(tv->stats->grpsize1), &(tv->stats->grpsize2), n, tv,
                       ti);
        ...
        if (canong_arg)
          memcpy(CurrPart->inv, BestCand->invlab, n * sizeof(int));
      } else {
        grouporderplus(g_arg, AuxCand, Spine[tv->maxtreelevel].part, &gensB,
                       &(tv->stats->grpsize1), &(tv->stats->grpsize2), n, tv,
                       ti);
      }
  ...
  if (tv->options->verbosity >= 2) {
    fprintf(outfile,
            "group time: %.2f, %.2f, %.2f; total: %.2f; exp_paths time: %.2f; "
            "aut_check time: %.2f\n%lu refinement%s interrupted by trace "
            "comparison (%s); special cells: %d\n------",
            tv->schreier1, tv->schreier2, tv->schreier3,
            tv->schreier1 + tv->schreier2 + tv->schreier3, tv->expaths,
            tv->autchk, SS(tv->stats->interrupted, "", "s"),
            (tv->options->strategy == 0 ? "breadth-first" : "depth-first"),
            tv->specialgens);
    PRINT_RETURN
  }
  if (tv->options->verbosity >= 3)
    fprintf(outfile,
            "CPYCAND(0): %d, ID<-TMPORB(1): %d, LAB(2): %d, PART(3): %d, "
            "TEMP(4)->: %d, TEMP(5)<-: %d, CHKFORAUT(6): %d, ISAUT(7): %d, "
            "ContaTC: %d\n",
            tv->conta0, tv->conta1, tv->conta2, tv->conta3, tv->conta4,
            tv->conta5, tv->conta6, tv->conta7, tv->contatc);
...
int traces_vertexclass_refine(int n, int *lab, int *ptn, Candidate *Cand,
                              Partition *Part, int *RefArray) {
...
int traces_refine(Candidate *Cand, int n, Partition *Part,
                  struct TracesVars *tv, struct TracesInfo *ti, int num_indv,
                  boolean make_code) {

  int i, j, k, jk, sc, ind0, ind1, ind2, ind3, ind4, tlp1, labi;
  int value, iend, newcell;
  int HitClsInd, SplInd, SplCntInd, CStackInd, TraceInd, TraceCCInd,
      TraceStepsInd, SingInd;
  int j1int, iend1int;
  unsigned int longcode;
  int newtrace = FALSE;
  int Sparse = TRUE;
  int *lab, *cls, *InvLab, *TracePos, *SplitCell, *LabCell, *TraceEnd,
      Traceccend, *Tracestpend;
  ...
  memcpy(CStack + 1, TheTrace + SpineTL->trcstart,
         (Part->active) * sizeof(int));
  ...
  while (CStackInd > 0) {
    ...
    do {
      ...
      if (tv->options->weighted) {
        ...
        while ((iend1int > weightend) &&
               ((TheGraph[labi].w)[weightend] == currentweight)) {
          weightend++;
        }
      } else {
      ...
      if (!newtrace) {
        TraceCell =
            ((ind0 == TheTraceCC[TraceCCInd]) && (TraceCCInd < Traceccend));
      }
        ...
        if (ti->thegraphisparse) {
          ...
          if (SplInd) {
            ...
            for (sc = 0; sc < SplInd; sc++) { /* For each cell C to be split */
              ...
              /* According to the numbers of neighbors of C into the current
               * cell */
              /* compute how many vertices in C will be placed into the same new
               * cell */
              ...
              if (SplCntInd) {
                TRACE_CHECK(TheTraceSteps, TraceStepsInd, SplCntInd + n,
                            Tracestpend)
              }
              ...
              /* Split the cell C and update the information for sizes of new
               * cells */
              ...
              for (k = 0; k < SplCntInd; k++) {
                ...
                cls[i] = value;
                if ((StackMarkers[ind0] != tv->stackmark) &&
                    (value > BigCellSize)) {
                  BigCell = i;
          ...
          if (Sparse) {
            ...
            /* The list of cells with neighbors in the current cell is also
             * built */
            if (cls[ind0] == n) {
              for (i = 0; i < n; i++) {
            ...
            if (SplInd) {
              ...
              TRACE_CHECK(TheTraceSplNum, TraceCCInd, SplInd + 2 * n,
                          &Traceccend)
              ...
              for (j = 0; j < SplInd; j++) { /* For each cell C to be split */
                ...
                /* According to the numbers of neighbors of C into the current
                 * cell */
                /* compute how many vertices in C will be placed into the same
                 * new cell */
                for (i = ind0; i < ind1; i++) {
                ...
                if (SplCntInd) {
                  TRACE_CHECK(TheTraceSteps, TraceStepsInd, SplCntInd + 2 * n,
                              Tracestpend)
                }
            ...
            while (ind4 < n) { /* For each cell C with size(C) > 1 */
              ...
              if (cls[ind4] > 1) {
                ...
                if (SplCntInd) {
                  TRACE_CHECK(TheTraceSteps, TraceStepsInd, SplCntInd + 3 * n,
                              Tracestpend)
                  ...
                  for (k = 0; k < SplCntInd; k++) {
                    ...
                    if ((StackMarkers[ind4] != tv->stackmark) &&
                        (value > BigCellSize)) {
                      BigCell = i;
                      BigCellPos = CStackInd;
                      BigCellSize = cls[i];
            ...
            if (SplInd) {
              ...
              TRACE_CHECK(TheTraceSplNum, TraceCCInd, SplInd + 3 * n,
                          &Traceccend)
              if (newtrace) {
...
void traces_refine_notrace(Candidate *Cand, int n, Partition *Part,
...
void traces_refine_maketrie(Candidate *Cand, int n, Partition *Part,
                            struct TracesVars *tv, struct TracesInfo *ti) {
...
int traces_refine_comptrie(Candidate *Cand, int n, Partition *Part,
                           struct TracesVars *tv, struct TracesInfo *ti) {
  int i, j, k, jk, sc, ind0, ind1, ind2, ind3, labi;
  int value, iend, newcell;
  int HitClsInd, SplInd, SplCntInd, CStackInd;
...
int traces_refine_sametrace(Candidate *Cand, int n, Partition *Part,
                            struct TracesVars *tv, struct TracesInfo *ti) {
        ...
        if (ti->thegraphisparse) {
          ...
          if (SplInd) {
            ...
            for (sc = 0; sc < SplInd; sc++) { /* For each cell C to be split */
              ...
              if (SplCntInd) {
                SAMETRACE_CHECK(TheTraceSteps, TraceStepsInd, SplCntInd + n,
                                Tracestpend)
              }
          ...
          if (Sparse) {
            ...
            /* DENSE-SPARSE CASE */
            if (SplInd) {
              SAMETRACE_CHECK(TheTraceSplNum, TraceCCInd, SplInd + 2 * n,
                              &Traceccend)
              ...
              for (j = 0; j < SplInd; j++) { /* For each cell C to be split */
                ...
                if (SplCntInd) {
                  SAMETRACE_CHECK(TheTraceSteps, TraceStepsInd,
                                  SplCntInd + 2 * n, Tracestpend)
                }
            ...
            /* DENSE-DENSE CASE */
            if (SplInd) {
              SAMETRACE_CHECK(TheTraceSplNum, TraceCCInd, SplInd + 3 * n,
                              &Traceccend)
            } else {
              if (TraceCell) {
                return 0;
  ...
  Part->code = Cand->code = CLEANUP(longcode);
  if ((Cand->code != SpineTL->part->code) || (TraceInd != *TraceEnd))
    return FALSE;
  return TRUE;
}

void refine_tr(sparsegraph *sg, int *lab, int *ptn, int *numcells, int *code,
               TracesOptions *options_arg) {
  ...
  if (ti->regular) {
    ...
    if (tv->options->weighted) {
      ...
      CurrPart->cells =
          traces_vertexclass_refine(n, lab, ptn, CurrCand, CurrPart, sg->d);
    }
  }
...
void refine_tr_refine(Candidate *Cand, int n, Partition *Part,
                      struct TracesVars *tv, struct TracesInfo *ti) {
...
int CheckForAutomorphisms(Candidate *CurrCand, Candidate *NextCand,
                          struct TracesVars *tv, struct TracesInfo *ti, int m,
  ...
  while (CheckLevel <= CheckLevelEnd) {
    ...
    while (CheckAutList) {
      if (CheckAutList->do_it && lookup(CheckAutList->stnode) &&
          (CheckAutList != NextCand) && (CheckAutList != CurrCand)) {
        if (CheckAutList->code == NextCand->code) {
          SETMARK(Markers, tv->mark)
          if (Part->cells == n) {
            if (tv->permInd)
              ResetAutom(tv->permInd, n, tv);
            for (i = 0; i < n; i++) {
              arg = NextCand->lab[i];
              ...
              if (Part->cls[i] == 1) {
                ...
                for (j = i; j < i + Part->cls[i]; j++) {
                  ...
                  if (CellMarkers1[arg] != tv->markcell1) {
                    ...
                    while (
                        (CellMarkers2[CheckAutList->lab[k]] == tv->markcell2) &&
                        (k < i + Part->cls[i])) {
                      k++;
                    }
                    if (k < i + Part->cls[i]) {
                      ...
                      if (tv->preprocessed && Diff[arg])
                        MakeTree(arg, val, tv->input_graph, n, tv, TRUE);
                      tmp = FirstNeighbour(arg, NextCand, Part, CellMarkers1,
                                           tv->markcell1, &arg, n);
                      if (tmp) {
                        CellMarkers1[arg] = tv->markcell1;
                        tmp = FirstNeighbour(val, CheckAutList, Part,
                                             CellMarkers2, tv->markcell2, &val,
                                             n);
                        CellMarkers2[val] = tv->markcell2;
                        SETPAIRSAUT(arg, val)
                        if (tv->preprocessed && Diff[arg])
                          MakeTree(arg, val, tv->input_graph, n, tv, TRUE);
                        while (tmp) {
                          tmp = NextNeighbour(arg, NextCand, Part, CellMarkers1,
                                              tv->markcell1, &arg, n);
                          if (tmp) {
                            CellMarkers1[arg] = tv->markcell1;
                            tmp = NextNeighbour(val, CheckAutList, Part,
                                                CellMarkers2, tv->markcell2,
                                                &val, n);
                            if (tmp) {
                              CellMarkers2[val] = tv->markcell2;
                              SETPAIRSAUT(arg, val)
                              if (tv->preprocessed && Diff[arg])
                                MakeTree(arg, val, tv->input_graph, n, tv,
                                         TRUE);
                            }
                          }
                        }
          ...
          if (isautom_sg_pair((graph *)tv->input_graph, AUTPERM,
                              tv->options->digraph, m, n, tv)) {
            if (!findperm(gensB, AUTPERM, n)) {
#ifdef TIMINGS_TRACES
              if (tv->options->verbosity >= 2)
                tv->schreier3 -= CPUTIME;
#endif
              ...
              tv->stats->numgenerators++;
              orbjoin_sp_perm(tv->orbits, AUTPERM, OrbList, n,
                              &tv->stats->numorbits);
              ti->thegrouphaschanged = TRUE;
              ti->identitygroup = FALSE;
              ...
              if (tv->options->userautomproc) {
                (*tv->options->userautomproc)(tv->stats->numgenerators, AUTPERM,
                                              n);
              }
            } else {
              orbjoin_sp_perm(tv->orbits, AUTPERM, OrbList, n,
                              &tv->stats->numorbits);
              if (tv->options->verbosity >= 2) {
                fprintf(outfile, "[A*(%d,%d)] ", CheckLevel,
                        CheckAutList->name);
              }
            }
            TrieCandFrom = NULL;
            ...
            if (TrieCandFrom) {
              ...
              if ((TrieCheckFrom->level > 1) &&
                  (TrieCheckFrom->father != CurrCand->stnode)) {
                TrieCandFrom = CurrCand->stnode;
                TrieCheckFrom = TrieCheckFrom->father;
                while (TrieCandFrom->father != TrieCheckFrom->father) {
                  TrieCandFrom = TrieCandFrom->father;
            ...
            for (temp = 1; temp <= tv->tolevel; temp++) {
              if (CheckAutList->lab[Spine[temp].tgtpos] !=
                  NextCand->lab[Spine[temp].tgtpos]) {
                break;
              }
            }
              ...
              if (TempOrbits) {
                if (tv->compstage == 0) {
                  for (j = 0; j < tv->permInd; j++) {
                    orbjoin_sp_pair(TempOrbits, TempOrbList, n, PrmPairs[j].arg,
                                    PrmPairs[j].val, &numtemporbits);
                  }
                } else {
            ...
            case 0:
              if (tv->strategy && (tv->steps == 1)) {
                RemoveFromLevel(temp, tv->maxtreelevel - 1, tv->strategy,
                                FALSE);
                if (TrieCandFrom) {
            ...
            case 2:
              ...
              if (temp == tv->maxtreelevel) {
                ...
                if (CheckAutList->lab[Spine[temp].tgtpos] == AutomCount[1]) {
                  for (i = 1; i < AutomCount[0]; i++)
                    if (AutomCount[i] == tmp1)
                      break;
                  if (i == AutomCount[0]) {
                    AutomCount[AutomCount[0]++] =
                        TempOrbits[NextCand->lab[Spine[temp].tgtpos]];
                  }
                }
              }
              break;
...
int CheckForSingAutomorphisms(Candidate *CurrCand, Partition *NextPart,
                              Candidate *NextCand, struct TracesVars *tv,
                              struct TracesInfo *ti, int m, int n) {
  ...
  while (CheckAutList != NULL) {
    if (CheckAutList->do_it &&
        (CheckAutList->stnode->father == CurrCand->stnode)) {
      if (CheckAutList->firstsingcode == NextCand->firstsingcode) {
        if (tv->permInd)
          ResetAutom(tv->permInd, n, tv);
        if ((tv->tolevel == 1) && (Spine[0].part->cells == 1))
          tmp = 2;
        else
          tmp = tv->tolevel;
        if (TreeFyTwo(tmp, CheckAutList, NextCand, NextPart, n, tv, ti)) {
          if (isautom_sg((graph *)tv->input_graph, AUTPERM,
                         tv->options->digraph, m, n)) {
            if (!findperm(gensB, AUTPERM, n)) {
#ifdef TIMINGS_TRACES
              if (tv->options->verbosity >= 2)
                tv->schreier3 -= CPUTIME;
              ...
              tv->lev_of_lastauto = tv->tolevel;
              if (tv->options->verbosity >= 2)
                fprintf(outfile, "[a(%d)] ", CheckAutList->name);
              if (tv->options->verbosity >= 2 && tv->options->writeautoms) {
                PRINT_RETURN
              }
              if (tv->options->writeautoms) {
            ...
            case 2:
              ...
              if ((temp == tv->maxtreelevel) &&
                  (CheckAutList->lab[Spine[temp].tgtpos] == AutomCount[1])) {
                tmp1 = TempOrbits[NextCand->lab[Spine[temp].tgtpos]];
                for (i = 1; i < AutomCount[0]; i++)
                  if (AutomCount[i] == tmp1)
                    break;
...
int CheckForMatching(Candidate *CurrCand, Candidate *NextCand, Partition *Part,
                     struct TracesVars *tv, struct TracesInfo *ti, int m,
                     int n) {
  ...
  while (CheckAutList != NULL) {
    if (CheckAutList->do_it && (CheckAutList->singcode == NextCand->singcode)) {
      ...
      CodeVerify = TRUE;
      for (i = Spine[TrieCheckFrom->level + 1].singstart; i < SpineTL->singend;
           i++) {
      ...
      if (CodeVerify) {
        if (isautom_sg_pair((graph *)tv->input_graph, AUTPERM,
          ...
          if (!findperm(gensB, AUTPERM, n)) {
            ...
            for (j = 0; j < tv->permInd; j++) {
              orbjoin_sp_pair(tv->orbits, OrbList, n, PrmPairs[j].arg,
                              PrmPairs[j].val, &tv->stats->numorbits);
            }
            ...
            tv->lev_of_lastauto = tv->tolevel;
            if (tv->options->verbosity >= 2)
              fprintf(outfile, "[M(%d)] ", CheckAutList->name);
            if (tv->options->verbosity >= 2 && tv->options->writeautoms) {
              PRINT_RETURN
            }
            if (tv->options->writeautoms) {
...
void Complete(sparsegraph *sg_orig, Candidate *Cand, Partition *Part, int cell,
              TracesVars *tv, double *grpsize1, int *grpsize2, permnode **ring,
              int n) {
  int i, j, k;
  int arg, val;
  int numtemporbits;
...
int CompStage0(Partition *CurrPart, Partition *NextPart, Candidate *CurrCand,
               Candidate *NextCand, int m, int n, struct TracesVars *tv,
  ...
  if (CurrCand->do_it) {
    if ((tv->orbits[temp] == temp) || tv->tolevel == 1) {
      ...
      if ((tv->group_level >= tv->tolevel) &&
          (FixedBase(fix, tv, CurrCand, 0, tv->fromlevel))) {
        tv->nfix = tv->fromlevel;
        tv->currorbit = findcurrorbits(gpB, tv->nfix);
      } else {
        if ((!ti->identitygroup) &&
            (((Spine[tv->fromlevel].liststart !=
               Spine[tv->fromlevel].listend) &&
...
#ifdef TIMINGS_TRACES
            if (tv->options->verbosity >= 2)
              tv->schreier1 -= CPUTIME;
#endif
            gom_level = getorbitsmin(fix, tv->nfix, gpB, &gensB, &tv->currorbit,
                                     CurrCand->lab + tv->tcell,
                                     CurrPart->cls[tv->tcell], n, TRUE);
#ifdef TIMINGS_TRACES
            if (tv->options->verbosity >= 2)
              tv->schreier1 += CPUTIME;
#endif
        ...
        for (k = tv->indivstart; k < tv->indivend; k++) {
          ...
          if (tv->currorbit[NextCand->vertex] != NextCand->vertex) {
            ...
            if (NextPart->cls[tv->tcell] == 2) {
              num_indv = 2;
              NextCand->singcode =
                  MASHCOMM(NextCand->singcode, CurrCand->lab[tv->tcell]);
              NextCand->singcode =
                  MASHCOMM(NextCand->singcode, CurrCand->lab[tv->tcell + 1]);
              ...
              num_indv = 1;
              NextCand->singcode =
                  MASHCOMM(NextCand->singcode, NextCand->vertex);
              if (SpineTL->singstart == SpineTL->singend) {
                Singletons[SpineTL->singend++] =
                    tv->tcell + NextPart->cls[tv->tcell] - 1;
              }
            }

            Individualize(NextPart, NextCand, NextCand->vertex, tv->tcell,
                          CurrPart->cells, SpineTL->tgtpos);
            tv->stats->numnodes++;
            tv->answ =
                traces_refine(NextCand, n, NextPart, tv, ti, num_indv, TRUE);
            switch (tv->answ) {
            ...
            case 1: /* The same trace has been found once more : add to the list
              ...
              NextCand->do_it = TRUE;
              if (tv->options->verbosity >= 2)
                PRINT_CANDIDATE(NextCand, tv->tolevel);
              ...
              if (tv->steps > 1) {
                if (tv->fromlevel <= tv->lev_of_lastauto) {
                  ...
                  closeloop = CheckForMatching(CurrCand, NextCand, NextPart, tv,
                                               ti, m, n);
                }
                if (NextCand->do_it) {
                  firstsing = TRUE;
                  ...
                  if (NextPart->cells != tv->finalnumcells) { /* 160712 */
                    ...
                    while (NextPart->cells < n) {
                      if (firstsing && BreakSteps[tv->tolevel]) {
                        ...
                        if (CheckForSingAutomorphisms(CurrCand, NextPart,
                                                      NextCand, tv, ti, m, n))
                          if (!NextCand->do_it) {
                            break;
                          }
                  ...
                  if (closeloop < tv->tolevel)
                    k = SpineTL->tgtend;
                  PRINT_RETURN
                  break;
                }

                if (!tv->strategy && !tv->options->getcanon &&
                    (tv->tolevel_tl == tv->tolevel + 1) &&
                    ((NextPart->cells > tv->finalnumcells) ||
                     (NextPart->cells == n))) { /* 160717 */
                  tv->levelfromCS0 = tv->tolevel;
                  tv->maxtreelevel = tv->tolevel_tl;
                  tv->finalnumcells = NextPart->cells;
                  if (tv->tolevel == 1) {
                    tv->newst_stage1 =
                        searchtrie_make(CurrCand, NextCand, n, tv);
                ...
                tv->newindex = 0;
                if (NextCand->do_it) {
                  closeloop = CheckForAutomorphisms(CurrCand, NextCand, tv, ti,
                                                    m, n, NextPart);
                  if (!NextCand->do_it && closeloop < tv->tolevel)
                    k = SpineTL->tgtend;
                }
#ifdef TIMINGS_TRACES
                if (tv->options->verbosity >= 2)
                ...
                if (BreakSteps[tv->tolevel]) {
                  NextCand->firstsingcode = NextCand->pathsingcode;
                  if (CheckForSingAutomorphisms(CurrCand, NextPart, NextCand,
                                                tv, ti, m, n))
                    if (!NextCand->do_it) {
                      PRINT_RETURN
                      break;
              ...
              PRINT_RETURN
              break;
            case 2: /* Delete the old list and start a new one: a better trace
                       has been found */
              ...
              if (NextPart->cells == n) {
                ...
                if (tv->options->usercanonproc != NULL) {
                  (*tv->options->usercanonproc)(
                      (graph *)tv->input_graph, NextCand->lab,
                      (graph *)tv->cangraph, tv->stats->canupdates,
                      NextCand->code, m, n);
                }
              }
              ...
              if (!tv->strategy && (tv->tolevel > 1) && !SpineTL->liststart) {
                ...
                if (NextPart->cells < tv->finalnumcells)
                  SpineTL->liststart->code = auxcode;
                ...
                if (tv->brkstpcount) {
                  ...
                  if (i < n)
                    SpineTL->liststart->firstsingcode = Spine[i].singcode;
                }
                ...
                if (tv->options->verbosity >= 2)
                  PRINT_CANDIDATE(SpineTL->liststart, tv->tolevel);
                PRINT_RETURN;

                if (!tv->strategy && !tv->options->getcanon &&
                    (tv->tolevel + 1 == tv->firstpathlength) &&
                    ((NextPart->cells > tv->finalnumcells) ||
                     (NextPart->cells == n))) {
                  tv->finalnumcells = NextPart->cells;
                ...
                SpTLliststart->pathsingcode = SpineTL->singcode =
                    SpTLliststart->singcode;
                SpTLliststart->firstsingcode = 0;

                PRINT_LINE
                if (tv->options->verbosity >= 2)
                  PRINT_CANDIDATE(SpTLliststart, tv->tolevel);
                ...
                if (tv->steps > 1) {
                  ...
                  while (has_nexttcell) {
                    ...
                    Spine[tv->tolevel_tl].singcode =
                        SpTLliststart->pathsingcode;
                    has_nexttcell =
                        TargetCellFirstPath(SpTLliststart, NextPart, tv);
                    PRINT_EXPPATHSTEP(SpTLliststart, TRUE)
                  }
                  if (NextPart->cells < n) {
                    PRINTF2("CStage0 3: %d\n", tv->finalnumcells);
                    tv->finalnumcells =
                        minint(NextPart->cells, tv->finalnumcells); /* 160712 */
                    tv->finalnumcells = NextPart->cells;
...
int CompStage1(Partition *CurrPart, Partition *NextPart, Candidate *CurrCand,
               Candidate *NextCand, int m, int n, struct TracesVars *tv,
               struct TracesInfo *ti) {
  ...
  for (k = tv->indivstart; k < tv->indivend; k++) {
    ...
    Individualize(NextPart, NextCand, CurrCand->lab[k], tv->tcell,
                  CurrPart->cells, SpineTL->tgtpos);
    ...
    traces_refine_maketrie(NextCand, n, NextPart, tv, ti);
    ...
    if ((NextPart->cells == tv->finalnumcells) || (NextPart->cells == n)) {
      ...
      /* ADD TO NEXT LEVEL */
      SpineTL->keptcounter++;
      if (!Spine[tv->tolevel].listend)
        COPYPART(Spine[tv->tolevel].part, NextPart);
      ADDTONEXTLEVEL;
      searchtrie_make(CurrCand, SpineTL->listend, n, tv);
    }
  } /* end for */
  ...
  gom_level = getorbitsmin(fix, tv->nfix, gpB, &gensB, &tv->currorbit,
                           CurrCand->lab + tv->tcell, CurrPart->cls[tv->tcell],
                           n, TRUE);
#ifdef TIMINGS_TRACES
  if (tv->options->verbosity >= 2)
    tv->schreier1 += CPUTIME;
...
int CompStage2(Partition *CurrPart, Partition *NextPart, Candidate *CurrCand,
               Candidate *NextCand, int m, int n, struct TracesVars *tv,
               struct TracesInfo *ti) {
  ...
  if (CurrCand->do_it) {
    if (tv->tolevel == 0) {
      ...
      for (k = tv->indivstart; k < tv->indivend; k++) {
        if ((tv->orbits[CurrCand->lab[k]] == CurrCand->lab[k]) &&
          ...
          if (NextPart->cls[tv->tcell] == 2) {
            ...
            NextCand->singcode =
                MASHCOMM(NextCand->singcode, CurrCand->lab[k] + labelorg);
          }
          ...
          if (tv->fromlevel == tv->maxtreelevel) {
            ...
            tv->answ = traces_refine_comptrie(NextCand, n, NextPart, tv, ti);
            if (tv->answ) {
              if (NextPart->cells != tv->finalnumcells) {
                CurrRefCells[NextPart->cells % n] +=
                    CurrOrbSize[CurrCand->lab[k]];
                if (CurrRefCells[NextPart->cells % n] >
                    MultRefCells[NextPart->cells % n]) {
                  k = n;
                  break;
                }
                continue;
            ...
            tv->answ = traces_refine_sametrace(NextCand, n, NextPart, tv, ti);
          ...
          if (tv->answ) {
            ...
            if ((NextPart->cells == tv->finalnumcells) ||
              ...
              autom = CheckForAutomorphisms(CurrCand, NextCand, tv, ti, temp, n,
                                            NextPart);
#ifdef TIMINGS_TRACES
              if (tv->options->verbosity >= 2)
                tv->autchk += CPUTIME;
      ...
      if (tv->cand_level ||
          ((tv->orbits[temp] == temp) &&
           ((tv->finalnumcells < n) ||
            (OrbSize[tv->orbits[temp]] >= OrbSize[tv->orbits[vertex]])))) {
        tv->fromlevel = tv->tolevel++;
        SpineFL = Spine + tv->fromlevel;
        SpineTL = Spine + tv->tolevel;
        tv->tcell = Spine[tv->tolevel].tgtcell;
        ...
        if (!ti->identitygroup) {
          ...
          if (ti->useTempOrbits1 && ti->useTempOrbits2) {
            ...
            if (ti->useTempOrbits1 && tv->fromlevel == tv->maxtreelevel) {
              ...
              gom_level = getorbitsmin(
                  fix, tv->nfix, gpB, &gensB, &tv->currorbit,
              ...
              if (gom_level < tv->nfix) {
                ...
                if (ti->useTempOrbits1) {
                  for (i = 1; i < AutomCount[0]; i++)
                    if (AutomCount[i] == CuOrb[tv->currorbit[CurrCand->vertex]])
                      break;
                  if (i < AutomCount[0]) {
        ...
        if (ti->minimalinorbits) {
          ...
          for (k = tv->indivstart; k < tv->indivend; k++) {
            ...
            if (NextPart->cls[tv->tcell] == 2) {
              ...
              NextCand->singcode =
                  MASHCOMM(NextCand->singcode, CurrCand->lab[k]);
            }
...
#endif
                temp = (tv->tolevel_tl == tv->tolevel + 1);
                autom = CheckForAutomorphisms(CurrCand, NextCand, tv, ti, temp,
                                              n, NextPart);
#ifdef TIMINGS_TRACES
                if (tv->options->verbosity >= 2)
                  tv->autchk += CPUTIME;
...
void ExperimentalStep(Partition *NextPart, Candidate *NextCand, TracesVars *tv,
                      TracesInfo *ti, int m, int n) {
  int i, iend, min, tmp;
  ...
  if (NextPart->cls[tv->tcellexpath] == 2) {
    NextCand->pathsingcode =
        MASHCOMM(NextCand->pathsingcode, NextCand->lab[tv->tcellexpath]);
    NextCand->pathsingcode =
        MASHCOMM(NextCand->pathsingcode, NextCand->lab[tv->tcellexpath + 1]);
  } else {
    NextCand->pathsingcode =
        MASHCOMM(NextCand->pathsingcode, NextCand->lab[tmp]);
  }

  tv->indiv_vtx = NextCand->lab[tmp];
  Individualize(NextPart, NextCand, NextCand->lab[tmp], tv->tcellexpath,
                NextPart->cells,
                tv->tcellexpath + NextPart->cls[tv->tcellexpath] - 1);

  tv->stats->numnodes++;
  if (tv->compstage == 0) {
    traces_refine_notrace(NextCand, n, NextPart, tv, ti);
  } else {
...
int FirstNeighbour(int vtx, Candidate *Cand, Partition *Part, int *Markers,
                   int mark, int *ngh, int n) {
  int *e_vtx;
  int i, k, deg;
...
int FixBase(int *fix, struct TracesVars *tv, Candidate *Cand, int from,
            int to) {
  int i, j, k, go, nfix;
...
boolean FixedBase(int *fix, struct TracesVars *tv, Candidate *Cand, int from,
                  int to) {
  int i, k, nfix;
...
void grouporderplus(sparsegraph *sg_orig, Candidate *Cand, Partition *Part,
                    permnode **ring, double *grpsize1, int *grpsize2, int n,
                    TracesVars *tv, TracesInfo *ti) {
      ...
      while (TrieNode->father) {
        if (Factorials[TrieNode->level]) {
          ...
          if (TrieNode->father->name) {
            ...
            temp = spinelementorbsize(tv->orbits,
                                      Spine[tv->maxtreelevel].liststart->lab +
                                          Spine[1].tgtcell,
                                      Spine[1].tgtsize, TrieNode->vtx);
  ...
  if (Part->cells < n) {
    ...
    for (i = 0; i < n; i += Part->cls[i]) {
      if (Part->cls[i] > 1) {
        if (TheGraph[Cand->lab[i]].d > 2)
          NonSingDegPlus1(Cand, Part, i, tv);
        NSFCells[NSFCInd++] = i;
      } else {
    ...
    for (ind = 0; ind < NSFCInd; ind++) {
      ...
      if (Part->cls[i] > 1) {
        tmp = Cand->lab[i];
        if ((TheGraph[tmp].d == 2) &&
            ((TheGraph[TheGraph[tmp].e[0]].d > 2) ||
             ((TheGraph[TheGraph[tmp].e[1]].d > 2)))) {
          n1 = TheGraph[tmp].e[0];
          n2 = TheGraph[tmp].e[1];
          if (TheGraph[n1].d > 2) {
            if (TheGraph[n2].d > 2) {
          ...
          if (counts == Part->cls[i]) {
            ...
            for (k = 0; k < StInd / counts; k++) {
              ...
              for (j0 = 0; j0 < counts - 1; j0++) {
                SETPAIRSAUTANDTREE(PERMSTACK[j0 * (StInd / counts) + k],
                                   PERMSTACK[(j0 + 1) * (StInd / counts) + k])
              }
          ...
          for (j = 0; j < StInd; j++) {
            ...
            if ((TheGraph[PERMSTACK[j]].d >= 0) &&
                (TheGraph[PERMSTACK[j]].d < sg_orig->d[PERMSTACK[j]])) {
    ...
    for (ind = 0; ind < NSFCInd; ind++) {
      ...
      if (Part->cls[i] > 1) {
        tmp = Cand->lab[i];
        if ((TheGraph[tmp].d == 2) &&
            ((TheGraph[TheGraph[tmp].e[0]].d == 1) ||
             ((TheGraph[TheGraph[tmp].e[1]].d == 1)))) {
          counts = 0;
          StInd = 0;
          for (j = i; j < i + Part->cls[i]; j++) {
            step = Cand->lab[j];
          ...
          if (counts == Part->cls[i]) {
            if (Part->inv[Cand->invlab[PERMSTACK[0]]] !=
                Part->inv[Cand->invlab[PERMSTACK[StInd / counts - 1]]]) {
              factorial(grpsize1, grpsize2, Part->cls[i]);
            } else {
        ...
        if (TheGraph[tmp].d == 2) {
              ...
              if ((w1 == k1) && (w2 == k2)) {
                ...
                for (w = 0; w < CYLGTH[i1]; w++) {
                  if (CYCOLR[w + k] == CYCOLR[((w + j) % CYLGTH[i1]) + k]) {
                    SETPAIRSAUTANDTREE(CYCLES[w + k],
                                       CYCLES[((w + j) % CYLGTH[i1]) + k])
                  } else {
                    break;
                  }
                }
              ...
              for (j = CYLGTH[i1] - 1; j > 0; j--) {
                ...
                if ((w1 == k1) && (w2 == k2)) {
                  ...
                  for (w = 0; w < CYLGTH[i1]; w++) {
                    SETPAIRSAUTANDTREE(
                        CYCLES[w + k],
                        CYCLES[((j - w + (w > j) * CYLGTH[i1]) % CYLGTH[i1]) +
                               k])
                  }
                  SPECIALGENERATORS
              ...
              if (counts > 1) {
                ...
                if (counts > 2) {
                  if (tv->permInd)
                    ResetAutom(tv->permInd, n, tv);
                  for (j0 = 0; j0 < CYLGTH[i1]; j0++) {
                    SETPAIRSAUTANDTREE(CYCLES[CYMULT[1] + j0],
                                       CYCLES[CYMULT[0] + j0])
                    if (tv->build_autom) {
                      SETPAIRSAUT(CYCLES[CYMULT[0] + j0],
                                  CYCLES[CYMULT[1] + j0])
                    }
                    MakeTree(CYCLES[CYMULT[0] + j0], CYCLES[CYMULT[1] + j0],
                             sg_orig, n, tv, FALSE);
                  }
                  SPECIALGENERATORS
                }
          ...
          for (c1 = 0; c1 < CyInd; c1++) {
            ...
            for (c2 = CYCPOS[c1]; c2 < c; c2++) {
              ...
              if ((TheGraph[CYCLES[c2]].d >= 0) &&
                  (TheGraph[CYCLES[c2]].d < sg_orig->d[CYCLES[c2]])) {
    ...
    for (ind = 0; ind < NSFCInd; ind++) {
      ...
      if (Part->cls[i] > 1) {
        tmp = Cand->lab[i];
        if ((TheGraph[tmp].d == 1) && (TheGraph[TheGraph[tmp].e[0]].d == 1) &&
            (i == Part->inv[Cand->invlab[TheGraph[tmp].e[0]]])) {
          factorial2(grpsize1, grpsize2, Part->cls[i]);
          ...
          if (Part->cls[i] == 2) {
            ...
            if (tv->permInd)
              ResetAutom(tv->permInd, n, tv);
            memmove(PERMSTACK + halfsize, PERMSTACK + halfsize + 1,
                    (halfsize - 1) * sizeof(int));
            temp = PERMSTACK[1];
          ...
          for (j = i; j < i + Part->cls[i]; j++) {
            ...
            if (Markers[temp] != tv->mark) {
              if ((TheGraph[temp].d >= 0) &&
                  (TheGraph[temp].d < sg_orig->d[temp])) {
              ...
              if ((TheGraph[TheGraph[temp].e[0]].d >= 0) &&
                  (TheGraph[TheGraph[temp].e[0]].d <
                   sg_orig->d[TheGraph[temp].e[0]])) {
    ...
    for (ind = 0; ind < NSFCInd; ind++) {
      ...
      if (Part->cls[i] > 1) {
        tmp = Cand->lab[i];
        if ((TheGraph[0].e != NULL) && (TheGraph[tmp].d != 0) &&
            (TheGraph[tmp].d != numvertices - 1))
          nghcell = Part->inv[Cand->invlab[TheGraph[tmp].e[0]]];
        else
          nghcell = i;
        if ((TheGraph[tmp].d == 0) ||
            ((TheGraph[tmp].d == numvertices - 1) && (TheGraph[tmp].d > 2)) ||
            ((TheGraph[tmp].d == 1) && (TheGraph[TheGraph[tmp].e[0]].d == 1) &&
             (i < nghcell))) {
          do_ngh = FALSE;
          if ((TheGraph[tmp].d == 1) && (TheGraph[TheGraph[tmp].e[0]].d == 1) &&
              (i != nghcell)) {
          ...
          for (j = i; j < k; j++) {
            ...
            if ((TheGraph[Cand->lab[j]].d >= 0) &&
                (TheGraph[Cand->lab[j]].d < sg_orig->d[Cand->lab[j]])) {
              MakeCanTree(Cand->lab[j], sg_orig, n, Cand, Part, tv);
              if (do_ngh) {
                MakeCanTree(TheGraph[Cand->lab[j]].e[0], sg_orig, n, Cand, Part,
                            tv);
              }
            }
          }
...
void Individualize(Partition *NextPart, Candidate *NextCand, int K, int Tc,
                   int Cl, int Pos) {
  int i, j;
...
void Initialize_Traces_Variables(TracesVars *tv, TracesOptions *options_arg,
  ...
  if (tv->options->generators || tv->options->writeautoms ||
      tv->options->userautomproc)
    tv->build_autom = TRUE;
  else
  ...
  tv->mark = tv->stackmark = tv->treemark = tv->autmark = tv->markcell1 =
      tv->markcell2 = NAUTY_INFINITY - 1;
  tv->conta0 = tv->conta1 = tv->conta2 = tv->conta3 = tv->conta4 = tv->conta5 =
      tv->conta6 = tv->conta7 = tv->contatc = 0;
...
boolean isautom_sg_pair(graph *g, int *p, boolean digraph, int m, int n,
  ...
  for (k = 0; k < tv->permInd; ++k) {
    ...
    for (j = 0; j < di; ++j)
      AutMarkers[p[e[vi + j]]] = tv->autmark;
    for (j = 0; j < di; ++j)
      if (AutMarkers[e[vpi + j]] != tv->autmark) {
        return FALSE;
      }
  }
...
void MakeCanTree(int v1, sparsegraph *sg_orig, int n, Candidate *Cand,
                 Partition *Part, struct TracesVars *tv) {
...
void MakeTree(int v1, int v2, sparsegraph *sg, int n, struct TracesVars *tv,
              boolean forceautom) {
    ...
    for (j1 = deg0; j1 < deg1; j1++) {
      ...
      if ((TreeMarkers[ngh1] != tv->treemark) && (ngh1 != ngh2)) {
        ...
        if (ngh1 != ngh2) {
          ...
          orbjoin_sp_pair(tv->orbits, OrbList, n, ngh1, ngh2,
                          &tv->stats->numorbits);
        }
      }
    }
...
int NextNeighbour(int vtx, Candidate *Cand, Partition *Part, int *Markers,
                  int mark, int *ngh, int n) {
  int *e_vtx;
  int i, deg;
  int cell1;
...
int NonSingDegPlus1(Candidate *Cand, Partition *Part, int cell,
                    TracesVars *tv) {
...
void NonSingDegPlus2(Candidate *Cand, Partition *Part, int cell,
                     TracesVars *tv) {
...
void orbjoin_sp_pair(int *orbits, int *list, int n, int u, int v,
                     int *numorbs) {
  int j1, j2, k1, k2;
...
int Preprocess(sparsegraph *sg, permnode **ring, Candidate *Cand, int n,
               Partition *Part, struct TracesVars *tv) {

  int i, j, j0, k, curr_cell, ind, ind0, ind1, ind2;
  int *sge;
  int HitClsInd, labi, nghb, value, SplInd, SplCntInd, sc, iend, CStackInd,
      newcell, TraceInd;
            ...
            if (TheGraph[Cand->lab[ind1]].d != 1) {
              for (i = ind1; i < ind2; i++) {
                ...
                if (NghCounts[value] > 1) {
                  factorial(&(tv->stats->grpsize1), &(tv->stats->grpsize2),
                            NghCounts[value]);
                  if (tv->permInd)
                    ResetAutom(tv->permInd, n, tv);
...
searchtrie *searchtrie_make(Candidate *CurrCand, Candidate *NextCand, int n,
                            struct TracesVars *tv) {
...
trielist *searchtrie_new(int n, struct TracesVars *tv) {
  ...
  tv->strielist->triearray[0].father = tv->strielist->triearray[0].first_child =
      NULL;
  tv->strielist->triearray[0].next_sibling =
      tv->strielist->triearray[0].last_child = NULL;
...
boolean SelectNextLevel(int n, struct TracesVars *tv, struct TracesInfo *ti) {
  ...
  default:
    ...
    case 0:
      ...
      PRINTF2("Spine[tv->nextlevel].part->cells: %d; ",
              Spine[tv->nextlevel].part->cells);
      ...
      if ((Spine[tv->nextlevel].part->cells == tv->finalnumcells) ||
        ...
        if ((tv->group_level < tv->tolevel) && !ti->first_matching &&
          ...
          if (orbitcell) {
            ...
            if (tv->options->verbosity >= 2)
              tv->schreier1 -= CPUTIME;
#endif
            getorbitsmin(fix, tv->nfix, gpB, &gensB, &tv->currorbit, NULL, n, n,
                         TRUE);
#ifdef TIMINGS_TRACES
            if (tv->options->verbosity >= 2)
              tv->schreier1 += CPUTIME;
...
boolean TargetCell(Candidate *TargCand, Partition *Part, int n,
                   struct TracesVars *tv, int Lv) {
...
int TargetCellExpPath(Candidate *TargCand, Partition *Part,
                      struct TracesVars *tv) {
  int Lv, n;
  ...
  if (tv->tolevel_tl < tv->tcellevel) {
    ...
    if (Part->cls[tv->tcellexpath] == 1) {
      ...
      return 1 + ((Spine[tv->tolevel_tl].tgtcell >=
                   Spine[tv->tolevel_tl - 1].tgtcell) &&
                  (Spine[tv->tolevel_tl].tgtend <=
                   Spine[tv->tolevel_tl - 1].tgtend));
    }
  } else {
...
boolean TargetCellFirstPath(Candidate *TargCand, Partition *Part,
                            struct TracesVars *tv) {
  ...
  while (TCell < 0) {
    ...
    if (TCell < 0) {
      if (Lv == 0) {
        if (tv->answ == 2) {
          tv->finalnumcells =
              minint(Part->cells, tv->finalnumcells); /* 160712 */
  ...
  if (Lv != Lev) {
    ...
    if (Spine[tv->tolevel].liststart) {
      if (!Spine[tv->tolevel].liststart->firstsingcode) {
        Spine[tv->tolevel].liststart->firstsingcode =
            Spine[tv->tolevel].liststart->pathsingcode;
      }
    }
  }
  return TRUE;
...
boolean TreeFyTwo(int From, Candidate *Cand1, Candidate *Cand2, Partition *Part,
...
void WeightCodes(int n) {
  ...
  *(WArray[0].ref) = 0;
  for (i = 1; i < sumdegs; i++) {
    if ((WArray[i].weight != WArray[i - 1].weight) ||
        (VArray[i] != VArray[i - 1])) {
      ord++;
    }
    *(WArray[i].ref) = ord;
...
boolean TargetCellSmall(Candidate *TargCand, Partition *Part, int n,
                        struct TracesVars *tv, int Lv) {
...
int TargetCellExpPathSmall(Candidate *TargCand, Partition *Part,
                           struct TracesVars *tv) {
  int Lv, n;
...
boolean TargetCellFirstPathSmall(Candidate *TargCand, Partition *Part,
                                 struct TracesVars *tv) {
  int n, TCell, TCSize, TCell1, TCSize1;
  int Lv, i, Lev, vtx, vtx_d;
  int loopstart, loopend;
