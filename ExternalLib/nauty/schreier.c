...
static boolean filterschreier(schreier *, int *, permnode **, boolean, int,
                              int);
...
static void testispermutation(int id, int *p, int n)
 ...
 * The function value is the previous setting. */
{
  ...
  if (nfails <= 0)
    schreierfails = SCHREIERFAILS;
  else
    schreierfails = nfails;
...
static permnode *newpermnode(int n)
...
static schreier *newschreier(int n)
...
permnode *findpermutation(permnode *pn, int *p, int n)
...
static void addpermutationunmarked(permnode **ring, int *p, int n)
...
boolean addgenerator(schreier **gp, permnode **ring, int *p, int n)
...
boolean condaddgenerator(schreier **gp, permnode **ring, int *p, int n)
/* Add new permutation to group, unless it is discovered to be
 * already in the group.  It is is possible to be in the group
...
static void delpermnode(permnode **ring)
...
void deleteunmarked(permnode **ring)
...
static void clearvector(permnode **vec, permnode **ring, int n)
...
static void initschreier(schreier *sh, int n)
...
void newgroup(schreier **sh, permnode **ring, int n)
...
static void applyperm(int *wp, int *p, int k, int n)
/* Apply the permutation p, k times to each element of wp */
{
  ...
  if (k <= 5) {
    ...
    if (k == 1)
      ...
      for (i = 0; i < n; ++i)
        wp[i] = p[workpermA[wp[i]]];
    else if (k == 5)
      for (i = 0; i < n; ++i)
        wp[i] = p[p[workpermA[wp[i]]]];
  } else {
    m = SETWORDSNEEDED(n);
#if !MAXN
    DYNALLOC1(int, workpermA, workpermA_sz, n, "applyperm");
...
static boolean filterschreier(schreier *gp, int *p, permnode **ring,
      ...
      for (i = 0; i < n; ++i)
        if (vec[i] && !vec[workperm[i]]) {
          ...
          for (j = workperm[i]; !vec[j]; j = workperm[j]) {
            if (!curr) {
              if (!ingroup)
                addpermutation(ring, workperm, n);
              else
                addpermutationunmarked(ring, workperm, n);
              ingroup = TRUE;
              curr = *ring;
            }
            vec[j] = curr;
...
boolean expandschreier(schreier *gp, permnode **ring, int n)
...
int *getorbits(int *fix, int nfix, schreier *gp, permnode **ring, int n)
...
int getorbitsmin(int *fix, int nfix, schreier *gp, permnode **ring,
  ...
  *orbits = fixorbs = sh->orbits;
  ...
  if (cell) {
    for (icell = 1; icell < ncell; ++icell)
      if (fixorbs[cell[icell]] != fixorbs[cell[0]])
        break;
...
void pruneset(set *fixset, schreier *gp, permnode **ring, set *x, int m, int n)
...
int schreier_gens(permnode *ring)
/* Returns the number of generators in the ring */
{
  ...
  if (!ring)
    ...
    for (j = 1, pn = ring->next; pn != ring; pn = pn->next)
      ++j;
...
void dumpschreier(FILE *f, schreier *gp, permnode *ring, int n)
/* Dump the whole schreier structure to file f. */
{
  ...
  for (j = 0, sh = schreier_freelist; sh; sh = sh->next)
    ++j;
  for (k = 0, pn = permnode_freelist; pn; pn = pn->next)
    ++k;
  ...
  if (ring) {
    ...
    pn = ring;
    do {
      fprintf(f, "  %03x ref=%lu mk=%d alloc=%d p=", PNCODE(pn), pn->refcount,
              pn->mark, pn->nalloc);
      for (i = 0; i < n; ++i)
        fprintf(f, " %d", pn->p[i]);
      fprintf(f, "\n");
...
void grouporder(int *fix, int nfix, schreier *gp, permnode **ring,
