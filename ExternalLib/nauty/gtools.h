...
#define SWLONG(c, boool, val, id)                                              \
  if (sw == c) {                                                               \
    boool = TRUE;                                                              \
    arg_long(&arg, &val, id);                                                  \
  }
#define SWRANGE(c, sep, boool, val1, val2, id)                                 \
  if (sw == c) {                                                               \
    boool = TRUE;                                                              \
    arg_range(&arg, sep, &val1, &val2, id);                                    \
  }
#define SWREAL(c, boool, val, id)                                              \
  if (sw == c) {                                                               \
    boool = TRUE;                                                              \
    arg_double(&arg, &val, id);                                                \
  }
#define SWREALRANGE(c, sep, boool, val1, val2, id)                             \
  if (sw == c) {                                                               \
    boool = TRUE;                                                              \
    arg_doublerange(&arg, sep, &val1, &val2, id);                              \
  }
#define SWSEQUENCE(c, sep, boool, val, maxvals, numvals, id)                   \
  if (sw == c) {                                                               \
    boool = TRUE;                                                              \
    arg_sequence(&arg, sep, val, maxvals, &numvals, id);                       \
  }
#define SWSEQUENCEMIN(c, sep, boool, val, minvals, maxvals, numvals, id)       \
  if (sw == c) {                                                               \
    boool = TRUE;                                                              \
    arg_sequence_min(&arg, sep, val, minvals, maxvals, &numvals, id);          \
  }
...
#define CATMSG4(fmt, x1, x2, x3, x4)                                           \
  sprintf(msg + strlen(msg), fmt, x1, x2, x3, x4)
#define CATMSG5(fmt, x1, x2, x3, x4, x5)                                       \
  sprintf(msg + strlen(msg), fmt, x1, x2, x3, x4, x5)
...
extern graph *readgg_inc(FILE *, graph *, int, int *, int *, graph *, int, int,
                         boolean *);
...
extern void fcanonise_inv(graph *, int, int, graph *, char *,
                          void (*)(graph *, int *, int *, int, int, int, int *,
                                   int, boolean, int, int),
                          int, int, int, boolean);
extern void fcanonise_inv_sg(sparsegraph *, int, int, sparsegraph *, char *,
                             void (*)(graph *, int *, int *, int, int, int,
                                      int *, int, boolean, int, int),
                             int, int, int, boolean);
extern void setlabptn(int *, int *, int *, int);
extern void fgroup(graph *, int, int, char *, int *, int *);
extern void fgroup_inv(graph *, int, int, char *, int *, int *,
                       void (*)(graph *, int *, int *, int, int, int, int *,
                                int, boolean, int, int),
                       int, int, int);
