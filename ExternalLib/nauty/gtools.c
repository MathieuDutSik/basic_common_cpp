...
FILE *opengraphfile(char *filename, int *codetype, int assumefixed,
                    long position) {
  FILE *f;
  int c, bl, firstc;
  long i, l;
  ...
  if (filename == NULL) {
    ...
    if (filename[0] == 'c' && filename[1] == 'm' && filename[2] == 'd' &&
        filename[3] == ':') {
#if !HAVE_POPEN
      gt_abort(">E The \"cmd:\" option is not available in this version.\n");
#else
      filename += 4;
      while (*filename == ' ')
        ++filename;
  ...
  if (c != '>') {
    *codetype = firstc == ':' ? SPARSE6 : firstc == '&' ? DIGRAPH6 : GRAPH6;
    ...
    if (!bad_header &&
      ...
      bad_header = TRUE;
    if (!bad_header && c == 'g') {
      if ((c = GETC(f)) == EOF || c != 'r' || (c = GETC(f)) == EOF ||
          c != 'a' || (c = GETC(f)) == EOF || c != 'p' ||
          (c = GETC(f)) == EOF || c != 'h' || (c = GETC(f)) == EOF ||
          c != '6' || (c = GETC(f)) == EOF || c != '<' ||
          (c = GETC(f)) == EOF || c != '<')
        bad_header = TRUE;
      else
      ...
      if ((c = GETC(f)) == EOF || c != 'i' || (c = GETC(f)) == EOF ||
          c != 'g' || (c = GETC(f)) == EOF || c != 'r' ||
          (c = GETC(f)) == EOF || c != 'a' || (c = GETC(f)) == EOF ||
          c != 'p' || (c = GETC(f)) == EOF || c != 'h' ||
          (c = GETC(f)) == EOF || c != '6' || (c = GETC(f)) == EOF ||
          c != '<' || (c = GETC(f)) == EOF || c != '<')
        bad_header = TRUE;
      else
        *codetype = DIGRAPH6 | HAS_HEADER;
    } else if (!bad_header && c == 'e') {
      if ((c = GETC(f)) == EOF || c != 'd' || (c = GETC(f)) == EOF ||
          c != 'g' || (c = GETC(f)) == EOF || c != 'e' ||
          (c = GETC(f)) == EOF || c != '_' || (c = GETC(f)) == EOF ||
          c != 'c' || (c = GETC(f)) == EOF || c != 'o' ||
          (c = GETC(f)) == EOF || c != 'd' || (c = GETC(f)) == EOF ||
          c != 'e' || (c = GETC(f)) == EOF || c != '<' ||
          (c = GETC(f)) == EOF || c != '<')
        bad_header = TRUE;
      else
      ...
      if ((c = GETC(f)) == EOF || c != 'p' || (c = GETC(f)) == EOF ||
          c != 'a' || (c = GETC(f)) == EOF || c != 'r' ||
          (c = GETC(f)) == EOF || c != 's' || (c = GETC(f)) == EOF ||
          c != 'e' || (c = GETC(f)) == EOF || c != '6' ||
          (c = GETC(f)) == EOF || c != '<' || (c = GETC(f)) == EOF || c != '<')
        bad_header = TRUE;
      else
        *codetype = SPARSE6 | HAS_HEADER;
    } else if (!bad_header && c == 'p') {
      if ((c = GETC(f)) == EOF || c != 'l' || (c = GETC(f)) == EOF ||
          c != 'a' || (c = GETC(f)) == EOF || c != 'n' ||
          (c = GETC(f)) == EOF || c != 'a' || (c = GETC(f)) == EOF ||
          c != 'r' || (c = GETC(f)) == EOF || c != '_' ||
          (c = GETC(f)) == EOF || c != 'c' || (c = GETC(f)) == EOF ||
          c != 'o' || (c = GETC(f)) == EOF || c != 'd' ||
          (c = GETC(f)) == EOF || c != 'e')
        bad_header = TRUE;
      else {
        if ((c = GETC(f)) == EOF)
  ...
  if (*codetype & PLANARCODEANY) {
    fprintf(stderr, ">E opengraphfile: planar_code files can only be opened at "
                    "the start\n");
    *codetype = UNKNOWN_TYPE | HAS_HEADER;
    FUNLOCKFILE(f);
    fclose(f);
...
char *getecline(FILE *f) /* read an edge_code line */
...
void encodegraphsize(int n, char **pp)
...
void stringcounts(char *s, int *pn, size_t *pe)
...
void stringtograph(char *s, graph *g, int m)
...
void stringtograph_inc(char *s, graph *g, int m, graph *prevg, int prevn)
/* Convert string (graph6, digraph6 or sparse6 format) to graph,
...
*/
{
  ...
  if ((readg_line = gtools_getline(f)) == NULL)
    return NULL;
...
graph * /* read graph into nauty format */
readgg_inc(FILE *f, graph *g, int reqm, int *pm, int *pn, graph *prevg,
           int prevm, int prevn, boolean *digraph)
/* graph6, digraph6 and sparse6 formats are supported
   f = an open file
   g = place to put the answer (NULL for dynamic allocation)
   reqm = the requested value of m (0 => compute from n)
...
*/
{
  ...
  if (readg_code == INCSPARSE6) {
    if (prevg == NULL)
      gt_abort(">E readg_inc: missing prior\n");
    n = prevn;
    m = prevm;
  } else {
    n = graphsize(s);
...
graph * /* read undirected graph into nauty format */
readg_inc(FILE *f, graph *g, int reqm, int *pm, int *pn, graph *prevg,
          int prevm, int prevn)
/* graph6 and sparse6 formats are supported
   f = an open file
   g = place to put the answer (NULL for dynamic allocation)
...
void stringtosparsegraph(char *s, sparsegraph *sg, int *nloops)
 ...
 */
{
  ...
  if (!sgg)
    return NULL;
  if (digraph)
    gt_abort(">E read_sg_loops() can't handle digraphs,"
             " use read_sgg_loops()\n");
  return sgg;
}
 ...
 */
{
  ...
  if (!sgg)
    return NULL;
  if (digraph)
    gt_abort(">E read_sg() can't handle digraphs,"
             " use read_sgg_loops()\n");
  return sgg;
}
...
char *ntod6(graph *g, int m, int n)
...
char *ntos6(graph *g, int m, int n)
  ...
  for (j = 0; j < n; ++j) {
    ...
    for (i = 0; i <= j; ++i) {
      if (ISELEMENT(gj, i)) {
        if (p >= plim) {
          ii = p - gcode;
          DYNREALLOC(char, gcode, gcode_sz, 3 * (gcode_sz / 2) + 10000,
                     "ntos6");
          p = gcode + ii;
          plim = gcode + gcode_sz - 20;
        }
...
char *ntois6(graph *g, graph *prevg, int m, int n)
   ...
   prevg == NULL implies there is no prior graph */
{
  ...
  for (j = 0; j < n; ++j) {
    ...
    for (iw = 0; iw < nwords; ++iw) {
      gdiff = gj[iw] ^ pgj[iw];
      if (TIMESWORDSIZE(iw + 1) > j + 1)
        gdiff &= ALLMASK(SETBT(j + 1));
      while (gdiff) {
        TAKEBIT(i, gdiff);
        i += TIMESWORDSIZE(iw);
  ...
  for (j = 0; j < n; ++j) {
    ...
    for (l = 0; l < dj; ++l) {
      ...
      if (i <= j) {
        if (p >= plim) {
          ii = p - gcode;
          DYNREALLOC(char, gcode, gcode_sz, 5 * (gcode_sz / 4) + 1000,
                     "sgtos6");
          p = gcode + ii;
          plim = gcode + gcode_sz - 20;
        }
...
void writed6(FILE *f, graph *g, int m, int n)
...
void writes6(FILE *f, graph *g, int m, int n)
...
void writeis6(FILE *f, graph *g, graph *prevg, int m, int n)
...
void writeg6_sg(FILE *f, sparsegraph *g)
...
void writed6_sg(FILE *f, sparsegraph *g)
/* write undirected sparse graph to file in sparse6 format */
{
...
void writes6_sg(FILE *f, sparsegraph *g)
/* write undirected sparse graph to file in sparse6 format */
{
  char *s;
...
void writepc_sg(FILE *f, sparsegraph *sg)
...
#define BEPUT4(x)                                                              \
  w = (x);                                                                     \
  buff[j++] = (unsigned char)((w >> 24) & 0xFF);                               \
  ...
  for (i = 0; i < n; ++i) {
    ...
    do {
      if (bytes == 1)
        ...
        BEGET4(j);
      if (j == EOF)
        gt_abort(">E readpc_sg : error 4 on reading\n");
...
sparsegraph *readpcle_sg(FILE *f, sparsegraph *sg)
/* read a planar_code graph into sparse graph format
    *f = an open file
    *sg = place to put the answer (NULL for dynamic allocation)
        - must be initialised if not NULL
  ...
  for (i = 0; i < n; ++i) {
    ...
    do {
      if (bytes == 1)
        ...
        LEGET4(j);
      if (j == EOF)
        gt_abort(">E readpcle_sg : error 4 on reading\n");
...
int doublevalue(char **ps, double *l) {
  boolean neg, pos;
  double sofar, weight;
  char *s;
...
void arg_long(char **ps, long *val, char *id) {
  int code;
...
void arg_int(char **ps, int *val, char *id) {
  int code;
  long longval;
...
void arg_double(char **ps, double *val, char *id) {
  int code;
...
boolean strhaschar(char *s, int c)
...
void arg_range(char **ps, char *sep, long *val1, long *val2, char *id) {
  int code;
  char *s;
...
void arg_sequence(char **ps, char *sep, long *val, int maxvals, int *numvals,
                  char *id) {
  int code, ival;
  char *s;
...
void arg_sequence_min(char **ps, char *sep, long *val, int minvals, int maxvals,
                      int *numvals, char *id) {
  int code, ival;
  char *s;
...
void arg_doublerange(char **ps, char *sep, double *val1, double *val2,
                     char *id) {
  int code;
  char *s;
...
void writerange(FILE *f, int c, long lo, long hi) /* Write a range. */
{
  if (c != '\0')
    fprintf(f, "%c", c);
  if (lo != -NOLIMIT)
...
char *stringcopy(char *s) /* duplicate string */
{
  char *scopy;
  size_t i, len;
...
void gtools_check(int wordsize, int m, int n, int version) {
