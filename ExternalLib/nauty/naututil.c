...
boolean readinteger(FILE *f, int *p) {
  int c, ans, minus;
...
boolean readinteger_sl(FILE *f, int *p) {
  int c, ans, minus;
...
boolean readstring(FILE *f, char *s, int slen) {
  int c;
  char *slim;
...
void putset(FILE *f, set *set1, int *curlenp, int linelength, int m,
            boolean compress) {
  int slen, j1, j2;
  char s[40];
...
void putset_firstbold(FILE *f, set *set1, int *curlenp, int linelength, int m,
                      boolean compress) {
...
void readgraph(FILE *f, graph *g, boolean digraph, boolean prompt, boolean edit,
...
void readgraph_sg(FILE *f, sparsegraph *sg, boolean digraph, boolean prompt,
  ...
  while (!done) {
    ...
    if (ISDIGIT(c)) {
      ...
      case '\n':
        neg = FALSE;
        if (prompt)
          fprintf(PROMPTFILE, "%2d : ", vv + labelorg);
        break;
      case EOF:
      case '.':
        done = TRUE;
...
void readgraph_swg(FILE *f, sparsegraph *sg, boolean digraph, boolean prompt,
                   int linelength, int n) {
  int i, j, k, vv, ww, c;
  boolean neg, done;
  int *d, *e, *evi;
...
void putgraph(FILE *f, graph *g, int linelength, int m, int n) {
  int i, curlen;
  set *pg;
...
void putgraph_sg(FILE *f, sparsegraph *sg, int linelength) {
  int i, n, curlen, slen;
  int *d, *e;
  size_t *v, j;
  sg_weight *wt;
...
void putmapping(FILE *f, int *lab1, int org1, int *lab2, int org2,
                int linelength, int n) {
  int i, curlen, slen;
  char s[60];
...
void putorbits(FILE *f, int *orbits, int linelength, int n) {
  int i, j;
...
void putorbitsplus(FILE *f, int *orbits, int linelength, int n) {
  int i, j;
  int m, curlen, sz, slen;
  char s[20];
...
void putquotient(FILE *f, graph *g, int *lab, int *ptn, int level,
                 int linelength, int m, int n) {
  int i;
  char s[50];
...
void putquotient_sg(FILE *f, sparsegraph *g, int *lab, int *ptn, int level,
                    int linelength) {
  int i, m, n;
  char s[50];
  int ic, curlen, v, w, cell1, cell2, numcells, jc, csize, k;
...
void putptn(FILE *f, int *lab, int *ptn, int level, int linelength, int n) {
  int i;
  int curlen, m;
...
void putcanon(FILE *f, int *canonlab, graph *canong, int linelength, int m,
              int n) {
  int i;
...
void putcanon_sg(FILE *f, int *canonlab, sparsegraph *canong, int linelength) {
  int i, n;
...
void readptn(FILE *f, int *lab, int *ptn, int *numcells, boolean prompt,
             int n) {
  int i, j;
  int c, v1, v2, m;
...
void unitptn(int *lab, int *ptn, int *numcells, int n) {
  int i;
...
void individualise(int *lab, int *ptn, int level, int v, int *pos,
                   int *numcells, int n) {
  int i, j;
...
void cellstarts(int *ptn, int level, set *cell, int m, int n) {
  int i;
...
void fixit(int *lab, int *ptn, int *numcells, int fixedvertex, int n) {
  int i;
...
long sethash(set *s, int n, long seed, int key) {
  int i, j, lsh, rsh;
  ...
  for (i = 0;; ++i) {
    si = s[i];
    l = SWCHUNK0(si);
    res = (((res << lsh) ^ ((res >> rsh) & lshmask) ^ l) + salt) & 0x7FFFFFFFUL;
    res = FUZZ1(res);
    if ((j += 16) >= n)
      break;
#if WORDSIZE > 16
...
long listhash(int *x, int nx, long key) {
...
long hashgraph_sg(sparsegraph *sg, long key) {
  int n, i;
  int *d, *e;
  size_t *v;
...
long hashgraph(graph *g, int m, int n, long key) {
  int i;
  set *gi;
  unsigned long val, accum;
...
long hash(set *setarray, long length, int key) {
  long code;
  set *sptr;
...
void readperm(FILE *f, int *perm, boolean prompt, int n) {
  int nv;
...
void readvperm(FILE *f, int *perm, boolean prompt, int n, int *nv) {
  int i;
  int m, c, v1, v2;
...
void relabel(graph *g, int *lab, int *perm, graph *workg, int m, int n) {
  long li;
  int i;
...
void relabel_sg(sparsegraph *sg, int *lab, int *perm, sparsegraph *workg) {
  int i, n;
  sparsegraph *tempsg;
  sparsegraph tmp;
...
void sublabel(graph *g, int *perm, int nperm, graph *workg, int m, int n) {
  long li;
  int i, j, k;
  int newm;
  set *gi, *wgi;
...
int countcells(int *ptn, int level, int n) {
  int i, cnt;
...
int subpartition(int *lab, int *ptn, int n, int *perm, int nperm) {
  int i, j;
...
void sublabel_sg(sparsegraph *sg, int *perm, int nperm, sparsegraph *workg) {
  int i, j, k, n;
  size_t newnde, kk;
  sparsegraph *tempsg;
  sparsegraph tmp;
...
void copycomment(FILE *fin, FILE *fout, int delimiter) {
  int c, backslash;
...
void converse_sg(sparsegraph *g1, sparsegraph *g2) {
  int *e1, *d1, *e2, *d2;
  size_t *v1, *v2, j;
  int i, k, n;
...
void complement_sg(sparsegraph *g1, sparsegraph *g2) {
  int *e1, *d1, *e2, *d2;
  size_t *v1, *v2, j, ndec;
  int i, l, m, n;
  int loops;
  ...
  if (loops > 1)
    ndec = n * (size_t)n - g1->nde;
  else
    ndec = n * (size_t)n - n - g1->nde;
  ...
  for (i = 0; i < n; ++i) {
    EMPTYSET(workset, m);
    for (j = v1[i]; j < v1[i] + d1[i]; ++j)
      ADDELEMENT(workset, e1[j]);
    if (loops == 0)
      ADDELEMENT(workset, i);

    v2[i] = ndec;
    for (l = 0; l < n; ++l)
      if (!ISELEMENT(workset, l))
        e2[ndec++] = l;
    d2[i] = ndec - v2[i];
  }
  g2->nde = ndec;
...
void mathon_sg(sparsegraph *g1, sparsegraph *g2) {
  int *e1, *d1, *e2, *d2;
  size_t *v1, *v2, j;
  int i, k, m, n1, n2;
...
void mathon(graph *g1, int m1, int n1, graph *g2, int m2, int n2) {
  int i, j, ii, jj;
  long li;
  set *rowptr, *gp;
...
void rangraph(graph *g, boolean digraph, int invprob, int m, int n) {
  int i, j;
  long li;
  set *row, *col;
...
void rangraph2(graph *g, boolean digraph, int p1, int p2, int m, int n) {
  int i, j;
  long li;
  set *row, *col;
...
void rangraph2_sg(sparsegraph *sg, boolean digraph, int p1, int p2, int n) {
  int i, j, k;
  int *dd, *ee;
...
static void putsequence(FILE *f, int *x, int linelength, int n)
...
static void putnumbers(FILE *f, int *x, int linelength, int n)
...
void putdegs(FILE *f, graph *g, int linelength, int m, int n) {
  int i;
  graph *gp;
...
void putdegseq(FILE *f, graph *g, int linelength, int m, int n) {
  int i;
  graph *gp;
...
void putdegs_sg(FILE *f, sparsegraph *sg, int linelength) {
...
void putdegseq_sg(FILE *f, sparsegraph *sg, int linelength) {
  int i;
#if !MAXN
...
void complement(graph *g, int m, int n) {
  boolean loops;
  int i, j;
  graph *gp;
...
void converse(graph *g, int m, int n) {
  int i, j;
  graph *gi, *gj;
...
void naututil_check(int wordsize, int m, int n, int version) {
