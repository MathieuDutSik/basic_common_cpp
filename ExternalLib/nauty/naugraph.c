...
dispatchvec dispatch_graph = {
    isautom,    testcanlab,       updatecan,      refine, refine1, cheapautom,
    targetcell, naugraph_freedyn, naugraph_check, NULL,   NULL};
...
void updatecan(graph *g, graph *canong, int *lab, int samerows, int m, int n) {
  ...
  for (i = samerows, ph = GRAPHROW(canong, samerows, M); i < n; ++i, ph += M)
...
void refine(graph *g, int *lab, int *ptn, int level, int *numcells, int *count,
            set *active, int *code, int m, int n) {
...
void refine1(graph *g, int *lab, int *ptn, int level, int *numcells, int *count,
             set *active, int *code, int m, int n) {
  int i, c1, c2, labc1;
  setword x;
  int split1, split2, cell1, cell2;
...
static int bestcell(graph *g, int *lab, int *ptn, int level, int tc_level,
                    int m, int n) {
  int i;
  set *gp;
...
void densenauty(graph *g, int *lab, int *ptn, int *orbits, optionblk *options,
                statsblk *stats, int m, int n, graph *h) {
...
void naugraph_check(int wordsize, int m, int n, int version) {
