...
static TLS_ATTR boolean getcanon, digraph, writeautoms, domarkers, cartesian,
    doschreier;
static TLS_ATTR int linelength, tc_level, mininvarlevel, maxinvarlevel,
    invararg;
static TLS_ATTR void (*usernodeproc)(graph *, int *, int *, int, int, int, int,
                                     int, int);
static TLS_ATTR void (*userautomproc)(int, int *, int *, int, int, int);
static TLS_ATTR void (*userlevelproc)(int *, int *, int, int *, statsblk *, int,
                                      int, int, int, int, int);
static TLS_ATTR int (*usercanonproc)(graph *, int *, graph *, unsigned long,
                                     int, int, int);
static TLS_ATTR void (*invarproc)(graph *, int *, int *, int, int, int, int *,
                                  int, boolean, int, int);
...
void nauty(graph *g_arg, int *lab, int *ptn, set *active_arg, int *orbits_arg,
           optionblk *options, statsblk *stats_arg, set *ws_arg, int worksize,
           int m_arg, int n_arg, graph *canong_arg) {
  int i;
  int numcells;
  int retval;
  int initstatus;
  ...
  if (n_arg > MAXN || n_arg > WORDSIZE * m_arg) {
    stats_arg->errstatus = NTOOBIG;
    fprintf(ERRFILE, "nauty: need n <= min(%d,%d*m)\n\n", MAXM, WORDSIZE);
    return;
  }
#endif
  if (n_arg == 0) /* Special code for zero-sized graph */
  {
    ...
    (g_arg, &g, canong_arg, &canong, lab, ptn, active, options, &initstatus, m,
     n);
    if (initstatus)
    ...
    if (canong == NULL)
      canong = canong_arg;
    OPTCALL(dispatch.cleanup)
    (g_arg, &g, canong_arg, &canong, lab, ptn, options, stats_arg, m, n);
    return;
  }
...
#endif
  OPTCALL(dispatch.cleanup)
  (g_arg, &g, canong_arg, &canong, lab, ptn, options, stats, m, n);
...
#endif
{
  ...
  /* refine partition : */
  doref(g, lab, ptn, level, &numcells, &qinvar, workperm, active, &refcode,
        dispatch.refine, invarproc, mininvarlevel, maxinvarlevel, invararg,
        digraph, M, n);
  ...
  if (numcells != n) {
    ...
    maketargetcell(g, lab, ptn, level, tcell, &tcellsize, &tc, tc_level,
                   digraph, -1, dispatch.targetcell, M, n);
...
#if !MAXN
        rtnlevel =
            firstpathnode0(lab, ptn, level + 1, numcells + 1, tcnode_this);
#else
  ...
  OPTCALL(userlevelproc)
  (lab, ptn, level, orbits, stats, tv1, index, tcellsize, numcells, childcount,
   n);
  return level - 1;
}
...
static int
#if !MAXN
othernode0(int *lab, int *ptn, int level, int numcells, tcnode *tcnode_parent)
#else
...
#endif
{
  ...
  if (numcells < n && (eqlev_first == level || (getcanon && comp_canon >= 0))) {
    if (!getcanon || comp_canon < 0) {
      maketargetcell(g, lab, ptn, level, tcell, &tcellsize, &tc, tc_level,
                     digraph, firsttc[level], dispatch.targetcell, M, n);
      if (tc != firsttc[level])
        eqlev_first = level - 1;
    } else
      maketargetcell(g, lab, ptn, level, tcell, &tcellsize, &tc, tc_level,
  ...
  for (tv1 = tv = nextelement(tcell, M, -1); tv >= 0;
    ...
    if (tv == tv1) {
      ...
      if (doschreier)
        pruneset(fixedpts, gp, &gens, tcell, M, n);
    }
...
static void firstterminal(int *lab, int level) {
  int i;
...
static int processnode(int *lab, int *ptn, int level, int numcells) {
      ...
      if (getcanon) {
        ...
        if (comp_canon == 0) {
          if (level < canonlevel)
            ...
            (*dispatch.updatecan)(g, canong, canonlab, samerows, M, n);
            samerows = n;
            comp_canon = (*dispatch.testcanlab)(g, canong, lab, &sr, M, n);
          }
        }
        if (comp_canon == 0) {
          for (i = 0; i < n; ++i)
  ...
  case 1: /* lab is equivalent to firstlab */
    ...
    ++stats->numgenerators;
    OPTCALL(userautomproc)
    (stats->numgenerators, workperm, orbits, stats->numorbits, stabvertex, n);
    if (doschreier)
...
static void recover(int *ptn, int level) {
  int i;
...
void extra_autom(int *p, int n) {
  ...
  ++stats->numgenerators;
  OPTCALL(userautomproc)
  (stats->numgenerators, p, orbits, stats->numorbits, stabvertex, n);
}
...
void extra_level(int level, int *lab, int *ptn, int numcells, int tv1,
                 int index, int tcellsize, int childcount, int n) {
  MULTIPLY(stats->grpsize1, stats->grpsize2, index);
  if (domarkers)
    writemarker(level, tv1, index, tcellsize, stats->numorbits, numcells);
  OPTCALL(userlevelproc)
