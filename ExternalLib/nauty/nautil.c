...
int nextelement(set *set1, int m, int pos) {
  ...
  for (;;) {
    if (setwd != 0)
      return TIMESWORDSIZE(w) + FIRSTBITNZ(setwd);
    if (++w == m)
      return -1;
    setwd = set1[w];
  }
...
#define CONDNL(x)                                                              \
  if (linelength > 0 && curlen + (x) > linelength) {                           \
    putstring(f, "\n   ");                                                     \
    curlen = 3;                                                                \
  }
...
void doref(graph *g, int *lab, int *ptn, int level, int *numcells, int *qinvar,
           int *invar, set *active, int *code,
           void (*refproc)(graph *, int *, int *, int, int *, int *, set *,
                           int *, int, int),
           void (*invarproc)(graph *, int *, int *, int, int, int, int *, int,
                             boolean, int, int),
           int mininvarlev, int maxinvarlev, int invararg, boolean digraph,
           int m, int n) {
  int pw;
  ...
  if (invarproc != NULL && *numcells < n && level >= minlev &&
      level <= maxlev) {
    (*invarproc)(g, lab, ptn, level, *numcells, tvpos, invar, invararg, digraph,
                 M, n);
    EMPTYSET(active, m);
    for (i = n; --i >= 0;)
...
void maketargetcell(graph *g, int *lab, int *ptn, int level, set *tcell,
                    ...
                    int hint,
                    int (*targetcell)(graph *, int *, int *, int, int, boolean,
                                      int, int, int),
                    int m, int n) {
  int i, j, k;
...
void breakout(int *lab, int *ptn, int level, int tc, int tv, set *active,
              int m) {
  int i, prev, next;
...
void nautil_check(int wordsize, int m, int n, int version) {
