...
// Options
#define MPREAL_HAVE_MSVC_DEBUGVIEW // Enable Debugger Visualizer for "Debug"
                                   // builds in MSVC.
#define MPREAL_HAVE_DYNAMIC_STD_NUMERIC_LIMITS // Enable extended
                                               // std::numeric_limits<mpfr::mpreal>
                                               // specialization. Meaning that
                                               // "digits", "round_style" and
                                               // similar members are defined as
                                               // functions, not constants. See
                                               // std::numeric_limits<mpfr::mpreal>
...
#define IsInf(x)                                                               \
  std::isinf(                                                                  \
      x) // GNU C/C++ (and/or other compilers), just hope for C99 conformance
#endif
...
// Move semantic should be enabled with great care in multi-threading
// environments, especially if MPFR uses custom memory allocators. Everything
// should be thread-safe and support passing ownership over thread boundary.
#if (__has_feature(cxx_rvalue_references) ||                                   \
     defined(__GXX_EXPERIMENTAL_CXX0X__) || __cplusplus >= 201103L ||          \
     (defined(_MSC_VER) && _MSC_VER >= 1600) &&                                \
         !defined(MPREAL_DISABLE_MOVE_SEMANTIC))

#define MPREAL_HAVE_MOVE_SUPPORT

// Use fields in mpfr_t structure to check if it was initialized / set dummy
// initialization
...
// Detect support for explicit converters.
     ...
     (defined(__GXX_EXPERIMENTAL_CXX0X__) && __GNUC_MINOR >= 5) ||             \
...
#define MPFR_USE_INTMAX_T // Enable 64-bit integer types - should be defined
                          // before mpfr.h
...
public:
  ...
  inline static mp_rnd_t get_default_rnd() {
    return (mp_rnd_t)(mpfr_get_default_rounding_mode());
  }
  inline static mp_prec_t get_default_prec() {
  ...
  // shared = true allows to avoid deep copy, so that mpreal and 'u' share the
  // same data & pointers.
  mpreal(const mpfr_t u, bool shared = false);

  mpreal(const char *s, mp_prec_t prec = mpreal::get_default_prec(),
         int base = 10, mp_rnd_t mode = mpreal::get_default_rnd());
  mpreal(const std::string &s, mp_prec_t prec = mpreal::get_default_prec(),
         int base = 10, mp_rnd_t mode = mpreal::get_default_rnd());
  ...
  // Get raw pointers so that mpreal can be directly used in raw mpfr_*
  // functions
  ::mpfr_ptr mpfr_ptr();
  ...
  std::string toString(int n = -1, int b = 10,
  ...
  friend const mpreal root(const mpreal &v, unsigned long int k,
                           mp_rnd_t rnd_mode);
  friend const mpreal pow(const mpreal &a, const mpreal &b, mp_rnd_t rnd_mode);
  friend const mpreal pow(const mpreal &a, const mpz_t b, mp_rnd_t rnd_mode);
  friend const mpreal pow(const mpreal &a, const unsigned long int b,
                          mp_rnd_t rnd_mode);
  friend const mpreal pow(const mpreal &a, const long int b, mp_rnd_t rnd_mode);
  friend const mpreal pow(const unsigned long int a, const mpreal &b,
                          mp_rnd_t rnd_mode);
  friend const mpreal pow(const unsigned long int a, const unsigned long int b,
                          mp_rnd_t rnd_mode);
  friend const mpreal fabs(const mpreal &v, mp_rnd_t rnd_mode);

  friend const mpreal abs(const mpreal &v, mp_rnd_t rnd_mode);
  friend const mpreal dim(const mpreal &a, const mpreal &b, mp_rnd_t rnd_mode);
  friend inline const mpreal mul_2ui(const mpreal &v, unsigned long int k,
                                     mp_rnd_t rnd_mode);
  friend inline const mpreal mul_2si(const mpreal &v, long int k,
                                     mp_rnd_t rnd_mode);
  friend inline const mpreal div_2ui(const mpreal &v, unsigned long int k,
                                     mp_rnd_t rnd_mode);
  friend inline const mpreal div_2si(const mpreal &v, long int k,
                                     mp_rnd_t rnd_mode);
  ...
  friend const mpreal atan2(const mpreal &y, const mpreal &x,
                            mp_rnd_t rnd_mode);
  ...
  friend const mpreal hypot(const mpreal &x, const mpreal &y,
                            mp_rnd_t rnd_mode);

  friend const mpreal fac_ui(unsigned long int v, mp_prec_t prec,
                             mp_rnd_t rnd_mode);
  ...
  friend const mpreal fma(const mpreal &v1, const mpreal &v2, const mpreal &v3,
                          mp_rnd_t rnd_mode);
  friend const mpreal fms(const mpreal &v1, const mpreal &v2, const mpreal &v3,
                          mp_rnd_t rnd_mode);
  friend const mpreal agm(const mpreal &v1, const mpreal &v2,
                          mp_rnd_t rnd_mode);
  friend const mpreal sum(const mpreal tab[], const unsigned long int n,
                          int &status, mp_rnd_t rnd_mode);
  friend int sgn(const mpreal &v);

// MPFR 2.4.0 Specifics
#if (MPFR_VERSION >= MPFR_VERSION_NUM(2, 4, 0))
  friend int sinh_cosh(mpreal &s, mpreal &c, const mpreal &v,
                       mp_rnd_t rnd_mode);
  ...
  friend const mpreal
  urandom(gmp_randstate_t &state,
          mp_rnd_t rnd_mode); // use gmp_randinit_default() to init state,
                              // gmp_randclear() to clear
#endif

#if (MPFR_VERSION >= MPFR_VERSION_NUM(3, 1, 0))
  friend const mpreal
  grandom(gmp_randstate_t &state,
          mp_rnd_t rnd_mode); // use gmp_randinit_default() to init state,
                              // gmp_randclear() to clear
  ...
  // Constants
  // don't forget to call mpfr_free_cache() for every thread where you are using
  // const-functions
  ...
  friend const mpreal remainder(const mpreal &x, const mpreal &y,
                                mp_rnd_t rnd_mode);
  friend const mpreal remquo(const mpreal &x, const mpreal &y, int *q,
                             mp_rnd_t rnd_mode);
...
// MPFR < 2.4.2 Specifics
  ...
  inline void
  set_prec(mp_prec_t prec,
           mp_rnd_t rnd_mode =
               get_default_rnd()); // Change precision with rounding mode

  // Aliases for get_prec(), set_prec() - needed for compatibility with
  // std::complex<mpreal> interface
  inline mpreal &setPrecision(int Precision,
...
private:
  ...
  // mpfr::mpreal=<DebugView>, <mp[0]._mpfr_prec,u>bits    ; Show value &
  // precision
  //
  // at the beginning of
  ...
  // "Smart" resources deallocation. Checks if instance initialized before
  // deletion.
  void clear(::mpfr_ptr);
};
...
inline mpreal::mpreal(mpreal &&other) {
  mpfr_set_uninitialized(mpfr_ptr()); // make sure "other" holds null-pointer
...
inline mpreal &mpreal::operator=(mpreal &&other) {
  if (this != &other) {
    mpfr_swap(mpfr_ptr(), other.mpfr_ptr()); // destructor for "other" will be
                                             // called just afterwards
    MPREAL_MSVC_DEBUGVIEW_CODE;
  }
  return *this;
}
...
inline mpreal::mpreal(const unsigned long long int u, mp_prec_t prec,
                      mp_rnd_t mode) {
  mpfr_init2(mpfr_ptr(), prec);
...
inline mpreal::mpreal(const unsigned long int u, mp_prec_t prec,
                      mp_rnd_t mode) {
  mpfr_init2(mpfr_ptr(), prec);
...
inline mpreal::mpreal(const std::string &s, mp_prec_t prec, int base,
                      mp_rnd_t mode) {
  mpfr_init2(mpfr_ptr(), prec);
...
// Use SFINAE to restrict arithmetic operations instantiation only for numeric
// types This is needed for smooth integration with libraries based on
// expression templates, like Eigen.
...
//////////////////////////////////////////////////////////////////////////
// sqrt
const mpreal sqrt(const unsigned int v,
                  mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal sqrt(const long int v,
                  mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal sqrt(const int v, mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal sqrt(const long double v,
                  mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal sqrt(const double v,
                  mp_rnd_t rnd_mode = mpreal::get_default_rnd());

// abs
inline const mpreal abs(const mpreal &x,
...
const mpreal pow(const mpreal &a, const unsigned int b,
                 mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(const mpreal &a, const int b,
                 mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(const mpreal &a, const long double b,
                 mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(const mpreal &a, const double b,
                 mp_rnd_t rnd_mode = mpreal::get_default_rnd());

const mpreal pow(const unsigned int a, const mpreal &b,
                 mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(const long int a, const mpreal &b,
                 mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(const int a, const mpreal &b,
                 mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(const long double a, const mpreal &b,
                 mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(const double a, const mpreal &b,
                 mp_rnd_t rnd_mode = mpreal::get_default_rnd());
...
const mpreal pow(const unsigned int a, const long int b,
                 mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(const unsigned int a, const int b,
                 mp_rnd_t rnd_mode = mpreal::get_default_rnd());
...
const mpreal pow(const long int a, const unsigned int b,
                 mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(const long int a, const long int b,
                 mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(const long int a, const int b,
                 mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(const long int a, const long double b,
                 mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(const long int a, const double b,
                 mp_rnd_t rnd_mode = mpreal::get_default_rnd());

const mpreal pow(const int a, const unsigned long int b,
                 mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(const int a, const unsigned int b,
                 mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(const int a, const long int b,
                 mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(const int a, const int b,
                 mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(const int a, const long double b,
                 mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(const int a, const double b,
                 mp_rnd_t rnd_mode = mpreal::get_default_rnd());
...
const mpreal pow(const long double a, const long int b,
                 mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(const long double a, const int b,
                 mp_rnd_t rnd_mode = mpreal::get_default_rnd());

const mpreal pow(const double a, const double b,
                 mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(const double a, const unsigned long int b,
                 mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(const double a, const unsigned int b,
                 mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(const double a, const long int b,
                 mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(const double a, const int b,
                 mp_rnd_t rnd_mode = mpreal::get_default_rnd());
...
inline mpreal &mpreal::operator=(const char *s) {
  // Use other converters for more precise control on base & precision &
  // rounding:
  //
  //        mpreal(const char* s,        mp_prec_t prec, int base, mp_rnd_t
  //        mode) mpreal(const std::string& s,mp_prec_t prec, int base, mp_rnd_t
  //        mode)
  //
...
inline const mpreal operator+(const mpreal &a, const mpreal &b) {
  mpreal c(
      0, (std::max)(mpfr_get_prec(a.mpfr_ptr()), mpfr_get_prec(b.mpfr_ptr())));
  mpfr_add(c.mpfr_ptr(), a.mpfr_srcptr(), b.mpfr_srcptr(),
           mpreal::get_default_rnd());
  return c;
}
...
inline const mpreal operator-(const mpreal &a, const mpreal &b) {
  mpreal c(
      0, (std::max)(mpfr_get_prec(a.mpfr_ptr()), mpfr_get_prec(b.mpfr_ptr())));
  mpfr_sub(c.mpfr_ptr(), a.mpfr_srcptr(), b.mpfr_srcptr(),
           mpreal::get_default_rnd());
  return c;
}
...
inline const mpreal operator*(const mpreal &a, const mpreal &b) {
  mpreal c(
      0, (std::max)(mpfr_get_prec(a.mpfr_ptr()), mpfr_get_prec(b.mpfr_ptr())));
  mpfr_mul(c.mpfr_ptr(), a.mpfr_srcptr(), b.mpfr_srcptr(),
           mpreal::get_default_rnd());
  return c;
}
...
inline const mpreal operator/(const mpreal &a, const mpreal &b) {
  mpreal c(0, (std::max)(mpfr_get_prec(a.mpfr_srcptr()),
                         mpfr_get_prec(b.mpfr_srcptr())));
  mpfr_div(c.mpfr_ptr(), a.mpfr_srcptr(), b.mpfr_srcptr(),
           mpreal::get_default_rnd());
  return c;
}
...
inline mpreal &mpreal::operator<<=(const int u) {
  mpfr_mul_2si(mpfr_ptr(), mpfr_srcptr(), static_cast<long int>(u),
               mpreal::get_default_rnd());
  MPREAL_MSVC_DEBUGVIEW_CODE;
  return *this;
}
...
inline mpreal &mpreal::operator>>=(const int u) {
  mpfr_div_2si(mpfr_ptr(), mpfr_srcptr(), static_cast<long int>(u),
               mpreal::get_default_rnd());
  MPREAL_MSVC_DEBUGVIEW_CODE;
  return *this;
}
...
// WARNING:
//
// Please note that following checks for double-NaN are guaranteed to work only
// in IEEE math mode:
//
// isnan(b) =  (b != b)
// isnan(b) = !(b == b)  (we use in code below)
//
// Be cautions if you use compiler options which break strict IEEE compliance
// (e.g. -ffast-math in GCC). Use std::isnan instead (C++11).
...
inline bool isnan(const mpreal &op) {
  return (mpfr_nan_p(op.mpfr_srcptr()) != 0);
}
inline bool isinf(const mpreal &op) {
  return (mpfr_inf_p(op.mpfr_srcptr()) != 0);
}
inline bool isfinite(const mpreal &op) {
  return (mpfr_number_p(op.mpfr_srcptr()) != 0);
}
inline bool iszero(const mpreal &op) {
  return (mpfr_zero_p(op.mpfr_srcptr()) != 0);
}
inline bool isint(const mpreal &op) {
  return (mpfr_integer_p(op.mpfr_srcptr()) != 0);
}

#if (MPFR_VERSION >= MPFR_VERSION_NUM(3, 0, 0))
inline bool isregular(const mpreal &op) {
...
inline long mpreal::toLong(mp_rnd_t mode) const {
...
inline float mpreal::toFloat(mp_rnd_t mode) const {
  return mpfr_get_flt(mpfr_srcptr(), mode);
}
inline double mpreal::toDouble(mp_rnd_t mode) const {
...
inline long long mpreal::toLLong(mp_rnd_t mode) const {
...
inline std::string mpreal::toString(int n, int b, mp_rnd_t mode) const {
  // TODO: Add extended format specification (f, e, rounding mode) as it done in
  // output operator
  (void)b;
  (void)mode;
...
inline std::ostream &mpreal::output(std::ostream &os) const {
  ...
  char *s = NULL;
  if (!(mpfr_asprintf(&s, format.str().c_str(), mpfr::mpreal::get_default_rnd(),
                      mpfr_srcptr()) < 0)) {
    os << std::string(s);
    mpfr_free_str(s);
  }
...
inline mp_exp_t mpreal::get_exp() const { return mpfr_get_exp(mpfr_srcptr()); }
...
inline const mpreal frexp(const mpreal &x, mp_exp_t *exp,
                          mp_rnd_t mode = mpreal::get_default_rnd()) {
  mpreal y(x);
#if (MPFR_VERSION >= MPFR_VERSION_NUM(3, 1, 0))
...
inline const mpreal frexp(const mpreal &x, int *exp,
                          mp_rnd_t mode = mpreal::get_default_rnd()) {
  mp_exp_t expl;
...
inline const mpreal ldexp(const mpreal &v, mp_exp_t exp) {
  ...
  // rounding is not important since we are just increasing the exponent (=
  // exact operation)
...
inline mpreal copysign(const mpreal &x, const mpreal &y,
  ...
  mpfr_setsign(rop.mpfr_ptr(), x.mpfr_srcptr(), mpfr_signbit(y.mpfr_srcptr()),
               rnd_mode);
  return rop;
}
...
inline mpreal &setsignbit(mpreal &x, bool minus,
                          mp_rnd_t rnd_mode = mpreal::get_default_rnd()) {
...
inline const mpreal sqr(const mpreal &x,
                        mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(sqr);
}

inline const mpreal sqrt(const mpreal &x,
                         mp_rnd_t r = mpreal::get_default_rnd()) {
...
inline const mpreal dim(const mpreal &a, const mpreal &b,
                        mp_rnd_t r = mpreal::get_default_rnd()) {
...
inline const mpreal cbrt(const mpreal &x,
                         mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(cbrt);
}
inline const mpreal fabs(const mpreal &x,
                         mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(abs);
}
inline const mpreal abs(const mpreal &x, mp_rnd_t r) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(abs);
}
inline const mpreal log(const mpreal &x,
                        mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(log);
}
inline const mpreal log2(const mpreal &x,
                         mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(log2);
}
inline const mpreal log10(const mpreal &x,
                          mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(log10);
}
inline const mpreal exp(const mpreal &x,
                        mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(exp);
}
inline const mpreal exp2(const mpreal &x,
                         mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(exp2);
}
inline const mpreal exp10(const mpreal &x,
                          mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(exp10);
}
inline const mpreal cos(const mpreal &x,
                        mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(cos);
}
inline const mpreal sin(const mpreal &x,
                        mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(sin);
}
inline const mpreal tan(const mpreal &x,
                        mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(tan);
}
inline const mpreal sec(const mpreal &x,
                        mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(sec);
}
inline const mpreal csc(const mpreal &x,
                        mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(csc);
}
inline const mpreal cot(const mpreal &x,
                        mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(cot);
}
inline const mpreal acos(const mpreal &x,
                         mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(acos);
}
inline const mpreal asin(const mpreal &x,
                         mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(asin);
}
inline const mpreal atan(const mpreal &x,
                         mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(atan);
}

inline const mpreal logb(const mpreal &x,
                         mp_rnd_t r = mpreal::get_default_rnd()) {
  return log2(abs(x), r);
}
inline mp_exp_t ilogb(const mpreal &x) { return x.get_exp(); }

inline const mpreal acot(const mpreal &v,
                         mp_rnd_t r = mpreal::get_default_rnd()) {
  return atan(1 / v, r);
}
inline const mpreal asec(const mpreal &v,
                         mp_rnd_t r = mpreal::get_default_rnd()) {
  return acos(1 / v, r);
}
inline const mpreal acsc(const mpreal &v,
                         mp_rnd_t r = mpreal::get_default_rnd()) {
  return asin(1 / v, r);
}
inline const mpreal acoth(const mpreal &v,
                          mp_rnd_t r = mpreal::get_default_rnd()) {
  return atanh(1 / v, r);
}
inline const mpreal asech(const mpreal &v,
                          mp_rnd_t r = mpreal::get_default_rnd()) {
  return acosh(1 / v, r);
}
inline const mpreal acsch(const mpreal &v,
                          mp_rnd_t r = mpreal::get_default_rnd()) {
  return asinh(1 / v, r);
}

inline const mpreal cosh(const mpreal &x,
                         mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(cosh);
}
inline const mpreal sinh(const mpreal &x,
                         mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(sinh);
}
inline const mpreal tanh(const mpreal &x,
                         mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(tanh);
}
inline const mpreal sech(const mpreal &x,
                         mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(sech);
}
inline const mpreal csch(const mpreal &x,
                         mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(csch);
}
inline const mpreal coth(const mpreal &x,
                         mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(coth);
}
inline const mpreal acosh(const mpreal &x,
                          mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(acosh);
}
inline const mpreal asinh(const mpreal &x,
                          mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(asinh);
}
inline const mpreal atanh(const mpreal &x,
                          mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(atanh);
}

inline const mpreal log1p(const mpreal &x,
                          mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(log1p);
}
inline const mpreal expm1(const mpreal &x,
                          mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(expm1);
}
inline const mpreal eint(const mpreal &x,
                         mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(eint);
}
inline const mpreal gamma(const mpreal &x,
                          mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(gamma);
}
inline const mpreal tgamma(const mpreal &x,
                           mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(gamma);
}
inline const mpreal lngamma(const mpreal &x,
                            mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(lngamma);
}
inline const mpreal zeta(const mpreal &x,
                         mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(zeta);
}
inline const mpreal erf(const mpreal &x,
                        mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(erf);
}
inline const mpreal erfc(const mpreal &x,
                         mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(erfc);
}
inline const mpreal besselj0(const mpreal &x,
                             mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(j0);
}
inline const mpreal besselj1(const mpreal &x,
                             mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(j1);
}
inline const mpreal bessely0(const mpreal &x,
                             mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(y0);
}
inline const mpreal bessely1(const mpreal &x,
                             mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(y1);
}

inline const mpreal nextpow2(const mpreal &x,
                             mp_rnd_t r = mpreal::get_default_rnd()) {
...
inline const mpreal remquo(const mpreal &x, const mpreal &y, int *q,
                           mp_rnd_t rnd_mode = mpreal::get_default_rnd()) {
  long lq;
  mpreal a(0, (std::max)(y.getPrecision(), x.getPrecision()));
...
inline const mpreal besseljn(long n, const mpreal &x,
                             mp_rnd_t r = mpreal::get_default_rnd()) {
  mpreal y(0, x.getPrecision());
...
inline const mpreal besselyn(long n, const mpreal &x,
                             mp_rnd_t r = mpreal::get_default_rnd()) {
  mpreal y(0, x.getPrecision());
...
inline const mpreal sum(const mpreal tab[], const unsigned long int n,
                        int &status,
                        mp_rnd_t mode = mpreal::get_default_rnd()) {
...
inline const mpreal li2(const mpreal &x,
                        mp_rnd_t r = mpreal::get_default_rnd()) {
...
inline const mpreal rec_sqrt(const mpreal &v,
                             mp_rnd_t rnd_mode = mpreal::get_default_rnd()) {
  mpreal x(v);
...
#if (MPFR_VERSION >= MPFR_VERSION_NUM(3, 0, 0))
inline const mpreal digamma(const mpreal &x,
                            mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(digamma);
}
inline const mpreal ai(const mpreal &x,
                       mp_rnd_t r = mpreal::get_default_rnd()) {
...
inline const mpreal const_infinity(int sign = 1,
...
inline const mpreal rint(const mpreal &x,
                         mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(rint);
}
inline const mpreal rint_ceil(const mpreal &x,
                              mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(rint_ceil);
}
inline const mpreal rint_floor(const mpreal &x,
                               mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(rint_floor);
}
inline const mpreal rint_round(const mpreal &x,
                               mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(rint_round);
}
inline const mpreal rint_trunc(const mpreal &x,
                               mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(rint_trunc);
}
inline const mpreal frac(const mpreal &x,
                         mp_rnd_t r = mpreal::get_default_rnd()) {
...
#if (MPFR_VERSION >= MPFR_VERSION_NUM(3, 0, 0))
inline const mpreal urandom(gmp_randstate_t &state,
                            mp_rnd_t rnd_mode = mpreal::get_default_rnd()) {
  mpreal x;
...
#if (MPFR_VERSION >= MPFR_VERSION_NUM(3, 1, 0))
inline const mpreal grandom(gmp_randstate_t &state,
                            mp_rnd_t rnd_mode = mpreal::get_default_rnd()) {
  mpreal x;
#if (MPFR_VERSION >= MPFR_VERSION_NUM(4, 0, 0))
...
inline const mpreal pow(const mpreal &a, const long long b,
                        mp_rnd_t rnd_mode = mpreal::get_default_rnd()) {
  (void)rnd_mode;
  return pow(a, mpreal(b));
}

inline const mpreal pow(const mpreal &a, const unsigned long long b,
                        mp_rnd_t rnd_mode = mpreal::get_default_rnd()) {
  (void)rnd_mode;
  return pow(a, mpreal(b));
}
...
inline const mpreal pow(const mpreal &a, const unsigned int b,
                        mp_rnd_t rnd_mode) {
...
inline const mpreal pow(const mpreal &a, const long double b,
                        mp_rnd_t rnd_mode) {
...
inline const mpreal pow(const unsigned int a, const mpreal &b,
                        mp_rnd_t rnd_mode) {
...
inline const mpreal pow(const long double a, const mpreal &b,
                        mp_rnd_t rnd_mode) {
  return pow(mpreal(a), b, rnd_mode);
}
...
inline const mpreal pow(const unsigned long int a, const unsigned long int b,
                        mp_rnd_t rnd_mode) {
  mpreal x(a);
...
inline const mpreal pow(const unsigned long int a, const long double b,
                        mp_rnd_t rnd_mode) {
  return pow(a, mpreal(b), rnd_mode); // mpfr_ui_pow
}

inline const mpreal pow(const unsigned long int a, const double b,
                        mp_rnd_t rnd_mode) {
  return pow(a, mpreal(b), rnd_mode); // mpfr_ui_pow
}
...
inline const mpreal pow(const unsigned int a, const unsigned int b,
  ...
  return pow(static_cast<unsigned long int>(a),
             static_cast<unsigned long int>(b), rnd_mode); // mpfr_ui_pow_ui
}

inline const mpreal pow(const unsigned int a, const long int b,
                        mp_rnd_t rnd_mode) {
  if (b > 0)
    return pow(static_cast<unsigned long int>(a),
               static_cast<unsigned long int>(b), rnd_mode); // mpfr_ui_pow_ui
  else
    return pow(static_cast<unsigned long int>(a), mpreal(b),
               rnd_mode); // mpfr_ui_pow
}
...
inline const mpreal pow(const unsigned int a, const long double b,
                        mp_rnd_t rnd_mode) {
  return pow(static_cast<unsigned long int>(a), mpreal(b),
             rnd_mode); // mpfr_ui_pow
}

inline const mpreal pow(const unsigned int a, const double b,
                        mp_rnd_t rnd_mode) {
  return pow(static_cast<unsigned long int>(a), mpreal(b),
             rnd_mode); // mpfr_ui_pow
}
...
inline const mpreal pow(const long int a, const unsigned int b,
                        mp_rnd_t rnd_mode) {
  if (a > 0)
    return pow(static_cast<unsigned long int>(a),
               static_cast<unsigned long int>(b), rnd_mode); // mpfr_ui_pow_ui
  else
    return pow(mpreal(a), static_cast<unsigned long int>(b),
               rnd_mode); // mpfr_pow_ui
}
...
inline const mpreal pow(const long int a, const long double b,
                        mp_rnd_t rnd_mode) {
  if (a >= 0)
    return pow(static_cast<unsigned long int>(a), mpreal(b),
               rnd_mode); // mpfr_ui_pow
...
// pow long double
inline const mpreal pow(const long double a, const long double b,
                        mp_rnd_t rnd_mode) {
  return pow(mpreal(a), mpreal(b), rnd_mode);
}

inline const mpreal pow(const long double a, const unsigned long int b,
                        mp_rnd_t rnd_mode) {
  return pow(mpreal(a), b, rnd_mode); // mpfr_pow_ui
}

inline const mpreal pow(const long double a, const unsigned int b,
                        mp_rnd_t rnd_mode) {
  return pow(mpreal(a), static_cast<unsigned long int>(b),
             rnd_mode); // mpfr_pow_ui
}

inline const mpreal pow(const long double a, const long int b,
                        mp_rnd_t rnd_mode) {
  return pow(mpreal(a), b, rnd_mode); // mpfr_pow_si
}
...
inline const mpreal pow(const double a, const unsigned long int b,
                        mp_rnd_t rnd_mode) {
  return pow(mpreal(a), b, rnd_mode); // mpfr_pow_ui
}

inline const mpreal pow(const double a, const unsigned int b,
                        mp_rnd_t rnd_mode) {
  return pow(mpreal(a), static_cast<unsigned long int>(b),
             rnd_mode); // mpfr_pow_ui
}
...
// Thus standard algorithms will use efficient version of swap (due to Koenig
// lookup) Non-throwing swap C++ idiom:
// http://en.wikibooks.org/wiki/More_C%2B%2B_Idioms/Non-throwing_swap
namespace std {
inline const mpfr::mpreal &min(const mpfr::mpreal &a, const mpfr::mpreal &b,
                               bool omitnan = false) {
  if (omitnan) {
    if (isnan(a))
      return b;
...
inline const mpfr::mpreal &max(const mpfr::mpreal &a, const mpfr::mpreal &b,
                               bool omitnan = false) {
  if (omitnan) {
    if (isnan(a))
      return b;
...
template <> inline void swap(mpfr::mpreal &x, mpfr::mpreal &y) {
...
public:
  ...
  inline static mpfr::mpreal(min)(
      mp_prec_t precision = mpfr::mpreal::get_default_prec()) {
  ...
  // Following members should be constant according to standard, but they can be
  // variable in MPFR So we define them as functions here.
  //
  // This is preferable way for std::numeric_limits<mpfr::mpreal>
  // specialization. But it is incompatible with standard std::numeric_limits
  // and might not work with other libraries, e.g. boost. See below for
  ...
  inline static int
  digits10(mp_prec_t precision = mpfr::mpreal::get_default_prec()) {
  ...
  inline static int
  max_digits10(mp_prec_t precision = mpfr::mpreal::get_default_prec()) {
  ...
  //
  // These (default) values are preserved for compatibility with existing
  // libraries, e.g. boost. Change them accordingly to your application.
  //
  // For example, if you use 256 bits of precision uniformly in your program,
  // then: digits       = 256 digits10     = 77 max_digits10 = 78
  //
  // Approximate formula for decimal digits is: digits10 = floor(log10(2) *
  // digits). See bits2digits() for more details.
