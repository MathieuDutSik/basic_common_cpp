#ifndef SVG_FUNCTIONALITIES
#define SVG_FUNCTIONALITIES
...
void GeneralWriteSVGfile(std::string const& eFile, SVGplotDescription const& eSVGplot)
{
  ...
  auto GetQualityString=[&](SVGqualInfo const& eQual) -> std::string {
    std::string eRet="style=\"stroke:" + StringColor(eQual.color) + ";stroke-width:" + DoubleToString(eQual.Size) + "\"";
    if (eQual.MarkerEnd != "")
  ...
  auto GetQualityStringPolyline=[&](SVGqualInfoPolyline const& eQual) -> std::string {
    std::string eRet="style=\"fill:" + StringColor(eQual.colorfill) + ";stroke:" + StringColor(eQual.colorstroke) + ";stroke-width:" + DoubleToString(eQual.Size) + "\"";
    if (eQual.MarkerEnd != "")
      eRet += " marker-end=\"url(#" + eQual.MarkerEnd + ")\"";
    if (eQual.clip != "")
      eRet += " clip-path=\"url(#" + eQual.clip + ")\"";
  ...
  for (auto & eGeneral : eSVGplot.ListGeneral) {
    if (eGeneral.svgtype == SvgType::clip) {
      ...
      os << "      <rect x=\"" << eGeneral.clip.x << "\" y=\"" << eGeneral.clip.y << "\" width=\"" << eGeneral.clip.width << "\" height=\"" << eGeneral.clip.height << "\" />\n";
      os << "    </clipPath>\n";
      os << "  </defs>\n";
    }
    if (eGeneral.svgtype == SvgType::line) {
      ...
      os << "  <line x1=\"" << GetStringValueX(ePt.x) << "\" y1=\"" << GetStringValueY(ePt.y) << "\" x2=\"" << GetStringValueX(fPt.x) << "\" y2=\"" << GetStringValueY(fPt.y) << "\" " << GetQualityString(eGeneral.line.eQual) << " />\n";
    }
    if (eGeneral.svgtype == SvgType::polyline) {
      os << "<polyline points=\"";
      bool IsFirst=true;
