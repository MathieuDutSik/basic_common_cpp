...
#ifndef SRC_MATRIX_MAT_MATRIXFUND_H_
#define SRC_MATRIX_MAT_MATRIXFUND_H_
...
template <typename T>
inline void ColAddMul(MyMatrix<T> &M, int i, T const &q, int j) {
  int nbRow = M.rows();
  for (int u = 0; u < nbRow; u++)
    AddMul(M(u, i), q, M(u, j));
}
...
template <typename T> MyMatrix<T> ExtendToBasis(MyMatrix<T> const &M) {
  ...
  if (size_t(n_rows) != V.size()) {
    ...
    std::cerr << "rank=" << V.size() << " n_cols=" << n_cols
              << " n_row=" << n_rows << "\n";
    std::cerr << "V =";
    for (auto &val : V)
      std::cerr << " " << val;
